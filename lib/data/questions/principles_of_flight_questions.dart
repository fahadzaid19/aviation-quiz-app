// lib/data/questions/principles_of_flight_questions.dart
import '../models/question_model.dart';

class PrinciplesOfFlightQuestions {
  static List<Question> getQuestions() {
    return [
      Question(
        question: 'The angle of attack (aerodynamic angle of incidence) of an aerofoil is the angle between the:',
        options: [
          'chord line and the relative undisturbed airflow.',
          'bottom surface and the horizontal',
          'bottom surface and the relative airflow.',
          'bottom surface and the chord line.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In a stationary subsonic streamline flow pattern, if the streamlines converge, in this part of the pattern, the static pressure (I) will ...and the velocity (II) will ...:',
        options: [
          '(I) decrease, (II) increase.',
          '(I) increase, (II) increase.',
          '(I) increase, (II) decrease.',
          '(I) increase, (II) decrease.',
        ],
        correctAnswer: 1,
      ),
      Question(
        question: 'The SI units of air density (I) and force (II) are:',
        options: [
          '(I) kg / m3, (II) N.',
          '(I) kg / m², (II) kg.',
          '(I) N / m3, (II) N.',
          '(I) N / kg, (II) kg.',
        ],
        correctAnswer: 2,
      ),
      Question(
        question: 'The units of wing loading (I) W / S and (II) dynamic pressure q are:',
        options: [
          '(I) N / m², (II) N / m².',
          '(I) N / m3, (II) kg / m².',
          '(I) kg / m, (II) N / m².',
          '(I) N / m, (II) kg.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 2',
        ],
        correctAnswer: 3,
      ),
      Question(
        question: 'Which formula or equation describes the relationship between force (F), acceleration (a) and mass (m)?',
        options: [
          'F=m. a',
          'm=F.a',
          'a=F. m',
          'F=m / a',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Static pressure acts:',
        options: [
          'in all directions.',
          'only in direction of the flow.',
          'only perpendicular to the direction of the flow.',
          'only in the direction of the total pressure.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Lift is generated when:',
        options: [
          'the flow direction of a certain mass of air is changed.',
          'a certain mass of air is accelerated in its flow direction.',
          'a symmetrical aerofoil is placed in a high velocity air stream at zero',
          'angle of attack.',
          'a certain mass of air is retarded.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Consider the steady flow through a stream tube where the velocity of the stream is V.  An increase in temperature of the flow at a constant value of V will:',
        options: [
          'decrease the mass flow.',
          'increase the mass flow.',
          'not affect the mass flow.',
          'increase the mass flow when the tube is divergent in the direction of the',
          'flow.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 3',
        ],
        correctAnswer: 7,
      ),
      Question(
        question: 'Which one of the following statements about Bernoulli\'s theorem is correct?',
        options: [
          'The dynamic pressure increases as static pressure decreases.',
          'The dynamic pressure decreases as static pressure decreases.',
          'The total pressure is zero when the velocity of the stream is zero.',
          'The dynamic pressure is maximum in the stagnation point.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If in a two-dimensional incompressible and subsonic flow, the streamlines converge the static pressure in the flow will:',
        options: [
          'decrease.',
          'increase.',
          'not change.',
          'increase initially, then decrease.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Bernoulli\'s equation can be written as:',
        options: [
          '(pt= total pressure, ps = static pressure and q=dynamic pressure)',
          'pt = ps + q',
          'pt = ps - q',
          'pt = q - ps',
          'pt = ps / q',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 4',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements about boundary layers is correct?',
        options: [
          'The turbulent boundary layer has more kinetic energy than the laminar',
          'boundary layer.',
          'The turbulent boundary layer is thinner than the laminar boundary layer.',
          'The turbulent boundary layer gives a lower skin friction than the laminar',
          'boundary layer.',
          'The turbulent boundary layer will separate more easily than the laminar',
          'boundary layer.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Where on the surface of a typical aerofoil will flow separation normally start at high angles of attack?',
        options: [
          'upper side trailing edge.',
          'upper side leading edge.',
          'lower side trailing edge.',
          'lower side leading edge.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'On an asymmetrical, single curve aerofoil, in subsonic airflow, at low angle of attack, when the angle of attack is increased, the centre of pressure will (assume a conventional transport aeroplane):',
        options: [
          'move forward.',
          'move aft.',
          'remain matching the airfoil aerodynamic centre.',
          'remain unaffected.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 5',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The Cl - alpha curve of a positive cambered aerofoil intersects with the vertical axis of the Cl - alpha graph:',
        options: [
          'above the origin.',
          'in the origin.',
          'below the origin.',
          'nowhere.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The angle of attack of a two dimensional wing section is the angle between:',
        options: [
          'the chord line of the aerofoil and the free stream direction.',
          'the chord line of the aerofoil and the fuselage centreline.',
          'the fuselage centreline and the free stream direction.',
          'the chord line and the camber line of the aerofoil.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The angle between the airflow (relative wind) and the chord line of an aerofoil is:',
        options: [
          'angle of attack.',
          'glide path angle.',
          'climb path angle.',
          'same as the angle between chord line and fuselage axis.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The angle between the aeroplane longitudinal axis and the chord line is the:',
        options: [
          'angle of incidence.',
          'glide path angle.',
          'angle of attack.',
          'climb path angle.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 6',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'With increasing angle of attack, the stagnation point will move (I) ...and the point of lowest pressure will move (II) ...Respectively (I) and (II) are:',
        options: [
          '(I) down, (II) forward.',
          '(I) up, (II) aft.',
          '(I) down, (II) aft.',
          '(I) up, (II) forward.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The aerodynamic centre of the wing is the point, where:',
        options: [
          'the pitching moment coefficient does not vary with angle of attack.',
          'changes of lift due to variations in angle of attack are constant.',
          'aerodynamic forces are constant.',
          'the aeroplane\'s lateral axis intersects with the centre of gravity.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '"Flutter" may be caused by:',
        options: [
          'distortion by bending and torsion of the structure causing increasing',
          'vibration in the resonance frequency.',
          'low airspeed aerodynamic wing stall.',
          'roll control reversal.',
          'high airspeed aerodynamic wing stall.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'On a swept wing aeroplane at low airspeed, the "pitch up" phenomenon:',
        options: [
          'is caused by wingtip stall.',
          'never occurs, since a swept wing is a "remedy" to pitch up.',
          'is caused by extension of trailing edge lift augmentation devices.',
          'is caused by boundary layer fences mounted on the wings.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 7',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Low speed pitch up is caused by the:',
        options: [
          'spanwise flow on a swept back wing.',
          'spanwise flow on a swept forward wing.',
          'wing tip vortex.',
          'Mach trim system.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The lift of an aeroplane of weight W in a constant linear climb with a climb angle (gamma) is approximately:',
        options: [
          'Wcos.gamma.',
          'W(1-sin.gamma).',
          'W(1-tan.gamma).',
          'W/cos.gamma.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The lift coefficient (CL) of an aeroplane in steady horizontal flight is 0.42. An increase in angle of attack of 1 degree increases CL by 0.1. A vertical up gust instantly changes the angle of attack by 3 degrees. The load factor will be:',
        options: [
          '1.71',
          '0.74',
          '1.49',
          '2.49',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The aeroplane drag in straight and level flight is lowest when the:',
        options: [
          'parasite drag is equal to the induced drag.',
          'parasite drag equals twice the induced drag.',
          'induced drag is equal to zero.',
          'induced drag is lowest.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 8',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Considering a positive cambered aerofoil, the pitching moment when Cl=0 is:',
        options: [
          'negative (nose-down).',
          'equal to zero.',
          'infinite',
          'positive (nose-up).',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'On a symmetrical aerofoil, the pitching moment for which Cl=0 is:',
        options: [
          'zero',
          'equal to the moment coefficient for stabilized angle of attack.',
          'positive (pitch-up)',
          'negative (pitch-down)',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'On a un-swept wing, when the aerofoil is accelerated from subsonic to supersonic speeds, the aerodynamic centre:',
        options: [
          'shifts from 25% to about 50% of the aerofoil chord.',
          'shifts aft by about 10%.',
          'remains unchanged.',
          'slightly shifts forward.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The lift coefficient (CL) of an aeroplane in steady horizontal flight is 0.4. An increase in angle of attack of 1 degree will increase CL by 0.09. A vertical up gust instantly changes the angle of attack by 5 degrees. The load factor will be:',
        options: [
          '2.13',
          '1.09',
          '2.0',
          '3.18',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 9',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane maintains straight and level flight while the IAS is doubled. The change in lift coefficient will be:',
        options: [
          'x 0.25',
          'x 2.0',
          'x 0.5',
          'x 4.0',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The effect of a positive wing sweep on static directional stability is as follows:',
        options: [
          'Stabilizing effect',
          'No effect',
          'Destabilizing dihedral effect',
          'Negative dihedral effect',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The effect on static stability of an aeroplane with a high wing as compared to a low wing is:',
        options: [
          'a positive dihedral effect',
          'a negative dihedral effect',
          'no effect as it is only used to improve aeroplane loading',
          'zero dihedral effect',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The lift coefficient (CL) of an aeroplane in steady horizontal flight is 0.35.  An increase in angle of attack of 1 degree would increase CL by 0.079.  If a vertical gust instantly changes the angle of attack by 2 degrees, the load factor will be:',
        options: [
          '1.45',
          '0.9',
          '0.45',
          '1.9',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 10',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When an aeroplane is flying at an airspeed which is 1.3 times its basic stalling speed, the coefficient of lift as a percentage of the maximum lift coefficient (CLmax) would be:',
        options: [
          '59%.',
          '130%.',
          '169%.',
          '77%.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The lift formula is:',
        options: [
          'L= CL 1/2 RHO V² S',
          'L= W',
          'L= CL 2 RHO V² S',
          'L= n W',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane is in a level turn, at a constant TAS of 300 kt, and a bank angle of 45°. Its turning radius is:',
        options: [
          '(given: g= 10 m/s²)',
          '2381 metres.',
          '4743 metres.',
          '9000 metres.',
          '3354 metres.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 11',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which one of the following statements about the lift-to-drag ratio in straight and level flight is correct?',
        options: [
          'At the highest value of the lift/drag ratio the total drag is lowest.',
          'The highest value of the lift/drag ratio is reached when the lift is zero.',
          'The lift/drag ratio always increases as the lift decreases.',
          'The highest value of the lift/drag ratio is reached when the lift is equal to',
          'the aircraft weight.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Drag is in the direction of - and lift is perpendicular to the:',
        options: [
          'relative wind/airflow.',
          'chord line.',
          'horizon.',
          'longitudinal axis.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If the nose of an aeroplane yaws left, this causes:',
        options: [
          'a roll to the left.',
          'a decrease in relative airspeed on the right wing.',
          'an increase in lift on the left wing.',
          'a roll to the right.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'At an aeroplane\'s minimum drag speed, what is the ratio between induced drag Di and parasite drag Dp? Di/Dp= 1/1 It varies between aeroplane types.',
        options: [
          '2/1',
          '1/2',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 12',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The correct drag formula is:',
        options: [
          'D= CD 1/2 RHO V² S',
          'D= CD 2 RHO V² S',
          'D= CD 1/2 RHO V S',
          'D= CD 1/2 1/RHO V² S',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The value of the parasite drag in straight and level flight at constant weight varies linearly with the:',
        options: [
          'square of the speed.',
          'speed.',
          'angle of attack.',
          'square of the angle of attack.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane accelerates from 80 kt to 160 kt at a load factor equal to 1. The induced drag coefficient (i) and the induced drag (ii) alter with the following factors:',
        options: [
          '(i) 1/16 (ii) 1/4',
          '(i) 1/4 (ii) 2',
          '(i) 1/2 (ii) 1/16',
          '(i) 4 (ii) 1/2',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the effect of high aspect ratio of an aeroplane\'s wing on induced drag?',
        options: [
          'It is reduced because the effect of wing-tip vortices is reduced.',
          'It is increased because high aspect ratio has greater frontal area.',
          'It is unaffected because there is no relation between aspect ratio and',
          'induced drag.',
          'It is increased because high aspect ratio produces greater downwash.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 13',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In what way do (1) induced drag and (2) parasite drag alter with increasing speed in straight and level flight ?',
        options: [
          '(1) decreases and (2) increases.',
          '(1) increases and (2) increases.',
          '(1) decreases and (2) decreases.',
          '(1) increases and (2) decreases.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following wing planforms produces the lowest induced drag? (all other relevant factors constant) Elliptical.',
        options: [
          'Rectangular.',
          'Tapered.',
          'Circular.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If flaps are deployed at constant IAS in straight and level flight, the magnitude of tip vortices will eventually: (flap span less than wing span) decrease.',
        options: [
          'increase.',
          'remain the same.',
          'increase or decrease, depending on the initial angle of attack.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The value of the induced drag of an aeroplane in straight and level flight at constant weight varies linearly with:',
        options: [
          '1/V²',
          'V²',
          'V',
          '1/V',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 14',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Induced drag at constant IAS is affected by:',
        options: [
          'aeroplane weight.',
          'aeroplane wing location.',
          'angle between wing chord and fuselage centre line.',
          'engine thrust.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following will reduce induced drag?',
        options: [
          'Elliptical lift distribution.',
          'Low aspect ratio.',
          'Flying at high angles of attack.',
          'Extending the flaps.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Induced drag is created by the:',
        options: [
          'spanwise flow pattern resulting in the tip vortices.',
          'interference of the air stream between wing and fuselage.',
          'separation of the boundary layer over the wing.',
          'propeller wash blowing across the wing.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Vortex generators:',
        options: [
          'transfer energy from the free airflow into the boundary layer.',
          'change the turbulent boundary layer into a laminar boundary layer.',
          'reduce the spanwise flow on swept wing.',
          'take kinetic energy out of the boundary layer to reduce separation.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 15',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'How does the total drag vary as speed is increased from stalling speed (VS) to maximum IAS (VNE) in a straight and level flight at constant weight?',
        options: [
          'Decreasing, then increasing.',
          'Decreasing.',
          'Increasing.',
          'Increasing, then decreasing.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A boundary layer fence on a swept wing will:',
        options: [
          'improve the low speed characteristics.',
          'improve the high speed characteristics.',
          'increase the critical Mach Number.',
          'improve the lift coefficient of the trailing edge flap.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In order to perform a steady level turn at constant speed in an aeroplane, the pilot must:',
        options: [
          'increase thrust/power and angle of attack.',
          'increase thrust/power and keep angle of attack unchanged.',
          'increase thrust/power and decrease angle of attack.',
          'increase angle of attack and keep thrust/power unchanged.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 16',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When an aeroplane with the centre of gravity forward of the centre of pressure of the combined wing / fuselage is in straight and level flight, the vertical load on the tailplane will be:',
        options: [
          'downwards.',
          'upwards.',
          'zero because in steady flight all loads are in equilibrium.',
          'downwards because it is always negative regardless of the position of',
          'the centre of gravity.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In a twin engine jet aeroplane (engines mounted below the low wings) the thrust is suddenly increased. Which elevator deflection will be required to maintain the pitching moment zero ?',
        options: [
          'Down.',
          'Up.',
          'No elevator movement will required because the thrust line of the',
          'engines remains unchanged.',
          'It depends on the position of the centre of gravity.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In which situation would the wing lift of an aeroplane in straight and level flight have the highest value ?',
        options: [
          'Forward centre of gravity and idle thrust.',
          'Aft centre of gravity and idle thrust.',
          'Forward centre of gravity and take-off thrust.',
          'Aft centre of gravity and take-off thrust.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 17',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane, with a C.G. location behind the centre of pressure of the wing can only maintain a straight and level flight when the horizontal tail loading is:',
        options: [
          'upwards.',
          'zero.',
          'downwards.',
          'upwards or downwards depending on elevator deflection.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The centre of gravity moving aft will:',
        options: [
          'increase the elevator up effectiveness.',
          'decrease the elevator up effectiveness.',
          'not affect the elevator up or down effectiveness.',
          'increase or decrease the elevator up effectiveness, depending on wing',
          'location.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If the total sum of moments about one of its axes is not zero, an aeroplane would:',
        options: [
          'experience an angular acceleration about that axis.',
          'be difficult to control.',
          'fly a path with a constant curvature.',
          'not be affected because the situation is normal.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'During landing of a low-winged jet aeroplane, the greatest elevator up deflection is normally required when the flaps are:',
        options: [
          'fully down and the centre of gravity is fully forward.',
          'up and the centre of gravity is fully forward.',
          'fully down and the centre of gravity is fully aft.',
          'up and the centre of gravity is fully aft.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 18',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Rotation about the lateral axis is called:',
        options: [
          'pitching.',
          'rolling.',
          'yawing.',
          'slipping.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Rolling is the rotation of the aeroplane about the:',
        options: [
          'longitudinal axis.',
          'vertical axis.',
          'lateral axis.',
          'wing axis.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane has static directional stability; in a side-slip to the right, initially the:',
        options: [
          'nose of the aeroplane tends to move to the right.',
          'right wing tends to go down.',
          'nose of the aeroplane will remain in the same direction.',
          'nose of the aeroplane tends to move to the left.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The centre of gravity of an aeroplane is in a fixed position forward of the neutral point. Speed changes cause a departure from the trimmed position.',
        options: [
          'Which of the following statements about the stick force stability is correct?',
          'An increase of 10kt from the trimmed position at low speed has more',
          'effect on the stick force than an increase of 10kt from the trimmed',
          'position at high speed.',
          'Increase of speed generates pull forces.',
          'Aeroplane nose up trim decreases the stick force stability.',
          'Stick force stability is not affected by trim.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 19',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The (1) stick force stability and the (2) manoeuvre stability are positively affected by:',
        options: [
          '(1) forward C.G. movement (2) forward CG. movement.',
          '(1) forward C.G. movement (2) trimming the aeroplane nose up.',
          '(1) aft C.G. movement (2) aft CG. movement.',
          '(1) trimming the aeroplane nose up (2) trimming the aeroplane nose up.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The value of the manoeuvre stability of an aeroplane is 150 N/g. The stick force required to achieve a load factor of 2,5 from steady level flight is:',
        options: [
          '225 N.',
          '375 N.',
          '450 N.',
          '150 N.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'For a normal stable aeroplane, the centre of gravity is located:',
        options: [
          'with a sufficient minimum margin ahead of the neutral point of the',
          'aeroplane.',
          'aft of the neutral point of the aeroplane.',
          'at the neutral point of the aeroplane.',
          'between the aft limit and the neutral point of the aeroplane.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The maximum aft position of the centre of gravity is, amongst others, limited by the:',
        options: [
          'required minimum value of the stick force per g.',
          'maximum longitudinal stability of the aeroplane.',
          'maximum elevator deflection.',
          'inability to achieve maximum rotation rate during take-off.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 20',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Longitudinal static stability is created by the fact that the:',
        options: [
          'centre of gravity is located in front of the neutral point of the aeroplane.',
          'centre of gravity is located in front of the leading edge of the wing.',
          'wing surface is greater than the horizontal tail surface.',
          'aeroplane possesses a large trim speed range.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Positive static stability of an aeroplane means that following a disturbance from the equilibrium condition:',
        options: [
          'the initial tendency is to return towards its equilibrium condition.',
          'the initial tendency is to diverge further from its equilibrium condition.',
          'the tendency is to move with an oscillatory motion of decreasing',
          'amplitude.',
          'the tendency is to move with an oscillatory motion of increasing',
          'amplitude.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Following a disturbance, an aeroplane oscillates about the lateral axis at a constant amplitude. The aeroplane is:',
        options: [
          'statically stable - dynamically neutral',
          'statically unstable - dynamically stable',
          'statically stable - dynamically unstable',
          'statically unstable - dynamically neutral',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 21',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement on dynamic longitudinal stability of a conventional aeroplane is correct?',
        options: [
          'Damping of the phugoid is normally very weak.',
          'Speed remains constant during one period of the phugoid.',
          'Period time of the phugoid is normally 5 sec.',
          'Damping of the short period oscillation is normally very weak.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The "short period mode" is an:',
        options: [
          'oscillation about the lateral axis.',
          'oscillation about the vertical axis.',
          'oscillation about the longitudinal axis.',
          'unstable movement of the aeroplane, induced by the pilot.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane that has positive static stability:',
        options: [
          'can be dynamically stable, neutral or unstable.',
          'is always dynamically stable.',
          'is never dynamically stable.',
          'is always dynamically unstable.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A statically unstable aeroplane is:',
        options: [
          'never dynamically stable.',
          'always dynamically stable.',
          'sometimes dynamically stable.',
          'sometimes dynamically unstable.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 22',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'One of the requirements for positive dynamic stability is:',
        options: [
          'positive static stability.',
          'a large deflection range of the stabilizer trim.',
          'a small cg range.',
          'an effective elevator.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements about dihedral is correct?',
        options: [
          'The "effective dihedral" of an aeroplane component means the',
          'contribution of that component to the static lateral stability.',
          'Effective dihedral is the angle between the 1/4-chord line and the lateral',
          'axis of the aeroplane.',
          'Dihedral contributes to dynamic but not to static lateral stability.',
          'Dihedral is necessary for the execution of slip-free turns.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'How does positive camber of an aerofoil affect static longitudinal stability ? It has no effect, because camber of the aerofoil produces a constant pitch down moment coefficient, independent of angle of attack.',
        options: [
          'positive effect, because the centre of pressure shifts rearward at',
          'increasing angle of attack.',
          'negative effect, because the lift vector rotates forward at increasing',
          'angle of attack.',
          'positive effect, because the lift vector rotates backward at increasing',
          'angle of attack.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 23',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following lists contain aeroplane design features that all increase static lateral stability?',
        options: [
          'High wing, sweep back, large and high vertical fin.',
          'Low wing, dihedral, elliptical wing planform.',
          'Fuselage mounted engines, dihedral, T-tail.',
          'Sweep back, under wing mounted engines, winglets.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '"Tuck under" is:',
        options: [
          'the tendency to nose down when speed is increased into the transonic',
          'flight regime.',
          'the tendency to nose up when speed is increased into the transonic',
          'flight regime.',
          'shaking of the control column at high Mach Number.',
          'the tendency to nose down when the control column is pulled back.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which wing design feature decreases the static lateral stability of an aeroplane?',
        options: [
          'Anhedral.',
          'Dihedral.',
          'High wing.',
          'Increased wing span.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '"Tuck under" may happen at:',
        options: [
          'high Mach numbers.',
          'low Mach numbers.',
          'all Mach numbers.',
          'only at low altitudes.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 24',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The manoeuvrability of an aeroplane is best when the:',
        options: [
          'C.G. is on the aft C.G. limit.',
          'speed is low.',
          'C.G. position is on the forward C.G. limit.',
          'flaps are down.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '"Tuck under" is caused by (i) which movement of the centre of pressure of the wing and (ii) which change of the downwash angle at the location of the stabilizer.',
        options: [
          '(i) aft (ii) decreasing',
          '(i) forward (ii) decreasing',
          '(i) aft (ii) increasing',
          '(i) forward (ii) increasing',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The effect of a ventral fin on the static stability of an aeroplane is as follows:',
        options: [
          '(1=longitudinal, 2=lateral, 3=directional)',
          '1: no effect, 2: negative, 3: positive',
          '1: positive, 2: negative, 3: negative',
          '1: negative, 2: positive, 3: positive',
          '1: no effect, 2: positive, 3: negative',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 25',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements about static lateral and directional stability is correct?',
        options: [
          'An aeroplane with an excessive static directional stability in relation to',
          'its static lateral stability, will be prone to spiral dive (spiral instability).',
          'The effects of static lateral and static directional stability are completely',
          'independent of each other because they take place about different axes.',
          'An aeroplane with an excessive static directional stability in relation to',
          'its static lateral stability, will be prone to "Dutch roll".',
          'Static directional stability can be increased by installing more powerful',
          'engines.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which moments or motions interact in a dutch roll?',
        options: [
          'Rolling and yawing.',
          'Pitching and yawing.',
          'Pitching and rolling.',
          'Pitching and adverse yaw.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Extension of FOWLER type trailing edge lift augmentation devices, will produce:',
        options: [
          'a nose-down pitching moment.',
          'no pitching moment.',
          'a nose-up pitching moment.',
          'a force which reduces drag.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 26',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'With increasing altitude and constant IAS the static lateral stability (1) and the dynamic lateral/directional stability (2) of an aeroplane with swept-back wing will:',
        options: [
          '(1) increase (2) decrease.',
          '(1) increase (2) increase.',
          '(1) decrease (2) decrease.',
          '(1) decrease (2) increase.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which one of the following systems suppresses the tendency to "Dutch roll"?',
        options: [
          'Yaw damper.',
          'Roll spoilers.',
          'Spoiler mixer.',
          'Rudder limiter.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which aeroplane behaviour will be corrected by a yaw damper ?',
        options: [
          'Dutch roll.',
          'Tuck under.',
          'Spiral dive.',
          'Buffeting.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Compared with level flight prior to the stall, the lift (1) and drag (2) in the stall change as follows:',
        options: [
          '(1) decreases (2) increases.',
          '(1) decreases (2) decreases.',
          '(1) increases (2) increases.',
          '(1) increases (2) decreases.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 27',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Entering the stall the centre of pressure of a straight (1) wing and of a strongly swept back wing (2) will:',
        options: [
          '(1) move aft, (2) move forward.',
          '(1) move aft, (2) move aft.',
          '(1) not move (2) move forward.',
          '(1) move aft, (2) not move.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-001.jpg A jet transport aeroplane weighing 100 tons carries out a steady level 50 degree bank turn at FL350.  The buffet free speed range from low speed to high speed buffet extends from:',
        options: [
          'M 0.69 to > M 0.84',
          'M 0.72 to > M 0.84',
          'M 0.65 to > M 0.84',
          'M 0.74 to M 0.84',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements about stall speed is correct ?',
        options: [
          'Decreasing the angle of sweep of the wing will decrease the stall speed.',
          'Increasing the angle of sweep of the wing will decrease the stall speed.',
          'Use of a T-tail will decrease the stall speed..',
          'Increasing the anhedral of the wing will decrease the stall speed.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements about the spin is correct?',
        options: [
          'During spin recovery the ailerons should be kept in the neutral position.',
          'An aeroplane is prone to spin when the stall starts at the wing root.',
          'In the spin, airspeed continuously increases.',
          'Every aeroplane should be designed such that it can never enter a spin.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 28',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements about a Mach trimmer is correct?',
        options: [
          'A Mach trimmer corrects the change in stick force stability of a swept',
          'wing aeroplane above a certain Mach number.',
          'A straight wing aeroplane always needs a Mach trimmer for flying at',
          'Mach numbers close to MMO.',
          'A Mach trimmer reduces the stick force stability of a straight wing',
          'aeroplane to zero at high Mach numbers.',
          'The Mach trimmer corrects the natural tendency of a swept wing',
          'aeroplane to pitch-up.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'During an erect spin recovery:',
        options: [
          'the ailerons are held in the neutral position.',
          'the control stick is moved side ways, against the angle of bank.',
          'the control stick is moved side ways, in the direction of the angle of',
          'bank.',
          'the control stick is pulled to the most aft position.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements about the stall of a straight wing aeroplane is correct?',
        options: [
          'Just before the stall the aeroplane will be have an increased nose-down',
          'tendency.',
          'Buffeting is the result of tailplane flow separation..',
          'The nose down effect is the result of increasing downwash, due to flow',
          'separation.',
          'The horizontal tail will stall at a higher speed than the wing.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 29',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'How is stall warning presented to the pilots of a large transport aeroplane ?',
        options: [
          'stick shaker and/or aerodynamic buffet.',
          'stick pusher.',
          'stall warning light only.',
          'aural warning only.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The vane of a stall warning system with a flapper switch is activated by the change of the:',
        options: [
          'stagnation point.',
          'centre of pressure.',
          'centre of gravity.',
          'point of lowest pressure.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which combination of design features is known to be responsible for deep stall?',
        options: [
          'Swept back wings and a T-tail.',
          'Straight wings and a T-tail.',
          'Swept back wings and wing mounted engines.',
          'Straight wings and aft fuselage mounted engines',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When a strongly swept back wing stalls and the wake of the wing contacts the horizontal tail, the effect on the stall behaviour can be a(n):',
        options: [
          'nose up tendency and/or lack of elevator response.',
          'nose down tendency.',
          'increase in sensitivity of elevator inputs.',
          'tendency to increase speed after initial stall.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 30',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The function of the stick pusher is:',
        options: [
          'to activate and push the stick forward at or beyond a certain value of',
          'angle of attack.',
          'to activate and push the stick forward prior to stick shaker.',
          'to vibrate the controls.',
          'to pull the stick, to avoid a high speed stall.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Dangerous stall characteristics, in large transport aeroplanes that require stick pushers to be installed, include:',
        options: [
          'excessive wing drop and deep stall.',
          'pitch down and yaw.',
          'distinct aerodynamic buffet.',
          'pitch down and increase in speed.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'By what percentage does the lift increase in a level turn at 45° angle of bank, compared to straight and level flight?',
        options: [
          '41%.',
          '19%.',
          '31%.',
          '52%.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In a steady level, co-ordinated  turn, the load factor n and the stalling speed VS will be:',
        options: [
          'n greater than 1, VS higher than in straight and level flight.',
          'n smaller than 1, VS lower than in straight and level flight.',
          'n greater than 1, VS lower than in straight and level flight.',
          'n smaller than 1, VS higher than in straight and level flight.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 31',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'On a wing fitted with a "fowler" type trailing edge flap, the "Full extended" position will produce:',
        options: [
          'an increase in wing area and camber.',
          'an unaffected wing area and increase in camber.',
          'an unaffected CD, at a given angle of attack.',
          'an increase in wing area only.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When flaps are extended in a straight and level flight at constant IAS, the lift coefficient will eventually:',
        options: [
          'remain the same.',
          'increase.',
          'decrease.',
          'first increase and then decrease.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When flaps are deployed at constant angle of attack the lift coefficient will:',
        options: [
          'increase.',
          'decrease.',
          'remain the same.',
          'vary as the square of IAS.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Trailing edge flap extension will:',
        options: [
          'decrease the critical angle of attack and increase the value of CLmax.',
          'increase the critical angle of attack and increase the value of CLmax.',
          'decrease the critical angle of attack and decrease the value of CLmax.',
          'increase the critical angle of attack and decrease the value of CLmax.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 32',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements about the difference between Krueger flaps and slats is correct?',
        options: [
          'Deploying a slat will form a slot, deploying a Krueger flap does not.',
          'Deploying a Krueger flap will form a slot, deploying a slat does not.',
          'Deploying a slat will increase critical angle of attack, deploying a',
          'Krueger flap does not.',
          'Deploying a Krueger flap will increase critical angle of attack, deploying',
          'a slat does not.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the most effective flap system?',
        options: [
          'Fowler flap.',
          'Split flap.',
          'Plain flap.',
          'Single slotted flap.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Deploying a Fowler flap, the flap will:',
        options: [
          'move aft, then turn down.',
          'turn down, then move aft.',
          'just move aft.',
          'just turn down.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A slotted flap will increase the CLmax by:',
        options: [
          'increasing the camber of the aerofoil and re-energising the airflow.',
          'decreasing the skin friction.',
          'increasing only the camber of the aerofoil.',
          'increasing the critical angle of attack.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 33',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In order to maintain straight and level flight at a constant airspeed, whilst the flaps are being retracted, the angle of attack must be:',
        options: [
          'increased',
          'decreased',
          'held constant',
          'increased or decreased depending upon the type of flap',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the purpose of an auto-slat system ?',
        options: [
          'extend automatically when a certain value of angle of attack is',
          'exceeded.',
          'provide automatically slat IN selection after take-off.',
          'ensures that the slats are always extended when the ground/flight',
          'system is in the "ground" position.',
          'assist the ailerons during rolling.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The function of the slot between an extended slat and the leading edge of the wing is to:',
        options: [
          'cause a venturi effect which energizes the boundary layer.',
          'allow space for vibration of the slat.',
          'reduce the wing loading.',
          'slow the air flow in the slot so that more pressure is created under the',
          'wing.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 34',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Given the following aeroplane configurations:',
        options: [
          '1. Clean wing',
          '2. Slats only extended',
          '3. Flaps only extended',
          'Place these configurations in order of stall angle of attack:',
          '3, 1, 2.',
          '1, 3, 2.',
          '2, 1, 3.',
          '2, 3, 1.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane has the following flap settings: 0°, 15°, 30° and 45°. Slats can also be selected. Which of the following selections will most adversely affect the CL/CD ratio?',
        options: [
          'Flaps from 30° to 45°.',
          'The slats.',
          'Flaps from 0° to 15°.',
          'Flaps from 15° to 30°.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'After take-off the slats (when installed) are always retracted later than the flaps. Why ?',
        options: [
          'Because SLATS EXTENDED gives a large decrease in stall speed with',
          'relatively less drag.',
          'Because SLATS EXTENDED provides a better view from the cockpit than',
          'FLAPS EXTENDED.',
          'Because VMCA with SLATS EXTENDED is more favourable compared to',
          'the FLAPS EXTENDED situation.',
          'Because FLAPS EXTENDED gives a large decrease in stall speed with',
          'relatively less drag.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 35',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Upon extension of a spoiler on a wing:',
        options: [
          'CD is increased and CL is decreased.',
          'only CL is decreased (CD remains unaffected).',
          'both CL and CD are increased.',
          'CD is increased, while CL remains unaffected.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When "spoilers" are used as speed brakes:',
        options: [
          'at same angle of attack, CD is increased and CL is decreased.',
          'CLmax of the polar curve is not affected.',
          'they do not affect wheel braking action during landing.',
          'at same angle of attack, CL remains unaffected.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'During initiation of a turn with speedbrakes extended, the roll spoiler function induces a spoiler deflection:',
        options: [
          'downward on the upgoing wing and upward on the downgoing wing.',
          'upward on the upgoing wing and downward on the downgoing wing.',
          'on the upgoing wing only.',
          'on the downgoing wing only.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Stick forces, provided by an elevator feel system, depend on:',
        options: [
          'elevator deflection, dynamic pressure.',
          'stabilizer position, static pressure.',
          'elevator deflection, static pressure.',
          'stabilizer position, total pressure.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 36',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'For a fixed-pitch propeller designed for cruise, the angle of attack of each blade, measured at the reference section:',
        options: [
          'is optimum when the aircraft is in a stabilized cruising flight.',
          'decreases when the aircraft speed decreases (with identical engine',
          'RPM).',
          'is lower in ground run than in flight (with identical engine RPM).',
          'is always positive during idling descent.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Why is a propeller blade twisted from root to tip?',
        options: [
          'To maintain a constant angle of attack along the whole length of the',
          'propeller blade.',
          'To ensure the angle of attack is greatest at the tip.',
          'To ensure that the root produces most thrust.',
          'To ensure that the tip produces most thrust.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Constant-speed propellers provide a better performance than fixed-pitch propellers because they:',
        options: [
          'produce an almost maximum efficiency over a wider speed range.',
          'have a higher maximum efficiency than a fixed-pitch propeller.',
          'produce a greater maximum thrust than a fixed-pitch propeller.',
          'have more blade surface area than a fixed-pitch propeller.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 37',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If you pull back the RPM lever of a constant speed propeller during a glide with idle power and constant speed, the propeller pitch will:',
        options: [
          'increase and the rate of descent will decrease.',
          'increase and the rate of descent will increase.',
          'decrease and the rate of descent will decrease.',
          'decrease and the rate of descent will increase.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If you push forward the RPM lever of a constant speed propeller during a glide with idle power and constant speed, the propeller pitch will:',
        options: [
          'decrease and the rate of descent will increase.',
          'increase and the rate of descent will decrease.',
          'increase and the rate of descent will increase.',
          'decrease and the rate of descent will decrease.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Propeller efficiency may be defined as the ratio between:',
        options: [
          'usable (power available) power of the propeller and shaft power.',
          'the thrust and the maximum thrust.',
          'the usable (power available) power and the maximum power.',
          'the thermal power of fuel-flow and shaft power.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An engine failure can result in a windmilling (1) propeller and a feathered (2) propeller. Which statement about propeller drag is correct?',
        options: [
          '(1) is larger than (2).',
          '(1) is equal to (2).',
          '(2) is larger than (1).',
          'impossible to say which one is largest.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 38',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When the blades of a propeller are in the feathered position:',
        options: [
          'the drag of the propeller is then minimal.',
          'the propeller produces an optimal windmilling RPM.',
          'the windmilling RPM is the maximum.',
          'the RPM is then just sufficient to lubricate the engine.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Increasing the number of propeller blades will:',
        options: [
          'increase the maximum absorption of power.',
          'increase the propeller efficiency.',
          'increase the noise level at maximum power.',
          'decrease the torque in the propeller shaft at maximum power.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The torque effect during the take off run in respect of a right hand propeller, when viewed from behind, will tend to:',
        options: [
          'roll the aeroplane to the left.',
          'pitch the aeroplane nose down.',
          'roll the aeroplane to the right.',
          'pitch the aeroplane nose up.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Gyroscopic precession of a propeller is induced by:',
        options: [
          'pitching and yawing.',
          'pitching and rolling.',
          'increasing RPM and yawing.',
          'increasing RPM and rolling.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 39',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Asymmetric propeller blade effect is mainly induced by:',
        options: [
          'the inclination of the propeller axis to the relative airflow.',
          'high speed.',
          'large angles of yaw.',
          'large angles of climb.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A propeller is turning to the right when viewed from behind. The asymmetric blade effect in the climb at low speed will:',
        options: [
          'yaw the aeroplane to the left.',
          'roll the aeroplane to the left.',
          'yaw the aeroplane to the right.',
          'roll the aeroplane to the right.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A jet aeroplane cruises buffet free at high constant altitude. Which type of stall can occur if this aeroplane decelerates during an inadvertent increase in load factor ?',
        options: [
          'Accelerated stall.',
          'Low speed stall.',
          'Shock stall.',
          'Deep stall.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which type of stall has the largest associated angle of attack?',
        options: [
          'Deep stall.',
          'Shock stall.',
          'Accelerated stall.',
          'Low speed stall.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 40',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which combination of speeds is applicable for structural strength in gust (clean configuration) ?',
        options: [
          '50 ft/sec and VC.',
          '66 ft/sec and VD.',
          '65 ft/sec at all speeds.',
          '55 ft/sec and VB.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The extreme right limitation for both V-n (gust and manoeuvre) diagrams is created by the speed:',
        options: [
          'VD',
          'VC',
          'Vflutter',
          'VMO',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The most important problem of ice accretion on a transport aeroplane during flight is:',
        options: [
          'reduction in CLmax.',
          'increase in weight.',
          'increase in drag.',
          'blocking of control surfaces.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The effects of very heavy rain (tropical rain) on the aerodynamic characteristics of an aeroplane are:',
        options: [
          'decrease of CLmax and increase of drag.',
          'decrease of CLmax and decrease of drag.',
          'increase of CLmax and increase of drag.',
          'increase of CLmax and decrease of drag.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 41',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'While flying under icing conditions, the largest ice build-up will occur, principally, on:',
        options: [
          'The frontal areas of the aircraft.',
          'The upper and lower surfaces on the rear of the wing.',
          'The upper and lower rudder surfaces.',
          'The pitot and static probes only.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A jet transport aeroplane is in a straight climb at a constant IAS and constant weight. The operational limit that may be exceeded is:',
        options: [
          'MMO.',
          'VMO.',
          'VA.',
          'MD.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The frontal area of a body, placed in a certain airstream is increased by a factor 3. The shape will not alter. The aerodynamic drag will increase with a factor:',
        options: [
          '3 .',
          '9 .',
          '6 .',
          '1.5 .',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 42',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'How does stalling speed (IAS) vary with altitude?',
        options: [
          'It remains constant at lower altitudes but increases at higher altitudes',
          'due to compressibility effects.',
          'It remains constant.',
          'It increases with increasing altitude, because the density decreases.',
          'It remains constant at lower altitudes but decreases at higher altitudes',
          'due to compressibility effects.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The aerodynamic drag of a body, placed in a certain airstream depends amongst others on:',
        options: [
          'The airstream velocity.',
          'The specific mass of the body.',
          'The weight of the body.',
          'The c.g. location of the body.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A body is placed in a certain airstream. The airstream velocity increases by a factor 4. The aerodynamic drag will increase with a factor:',
        options: [
          '16 .',
          '4 .',
          '8 .',
          '12 .',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 43',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What data may be obtained from the Buffet Onset Boundary chart?',
        options: [
          'The values of the Mach Number at which low speed and Mach Buffet',
          'occur at different weights and altitudes.',
          'The values of MMO at different weights and altitudes.',
          'The values of Mcrit at different weights and altitudes.',
          'The values of the Mach Number at which low speed and shock-stall',
          'occur at different weights and altitudes.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A body is placed in a certain airstream. The density of the airstream decreases to half of the original value. The aerodynamic drag will decrease with a factor:',
        options: [
          '2 .',
          '4 .',
          '8 .',
          '1.4 .',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Mcrit is the free stream Mach Number at which:',
        options: [
          'somewhere about the airframe Mach 1 is reached locally.',
          'Mach buffet occurs.',
          'shockstall occurs.',
          'the critical angle of attack is reached.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 44',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following (1) aerofoils and (2) angles of attack will produce the lowest Mcrit values?',
        options: [
          '(1) thick and (2) large.',
          '(1) thick and (2) small.',
          '(1) thin and (2) large.',
          '(1) thin and (2) small.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The point, where the aerodynamic lift acts on a wing is:',
        options: [
          'the centre of pressure.',
          'the centre of gravity location.',
          'the point of maximum thickness of the wing.',
          'the suction point of the wing.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which kind of flow separation occurs at the smallest angle of attack?',
        options: [
          'shockstall.',
          'Accelerated stall.',
          'low-speed stall.',
          'deep stall.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The location of the centre of pressure of a positive cambered wing at increasing angle of attack will:',
        options: [
          'shift forward.',
          'not shift.',
          'shift aft.',
          'shift in spanwise direction.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 45',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When the Mach number is slowly increased in straight and level flight the first shockwaves will occur:',
        options: [
          'on the upper surface at the wing root.',
          'on the lower surface of the wing.',
          'somewhere on the fin.',
          'somewhere on the horizontal tail.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The unit of density is:',
        options: [
          'kg/m³',
          'psi',
          'kg/cm²',
          'Bar',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The unit of measurement of pressure is:',
        options: [
          'psi',
          'kg/m³',
          'lb/gal',
          'kg/dm²',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The consequences of exceeding Mcrit in a swept-wing aeroplane may be:',
        options: [
          '(assume no corrective devices, straight and level flight)',
          'buffeting of the aeroplane and a tendency to pitch down.',
          'an increase in speed and a tendency to pitch up.',
          'engine unbalance and buffeting.',
          'buffeting of the aeroplane and a tendency to pitch up.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 46',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The boundary layer of a wing is:',
        options: [
          'a layer on the wing in which the stream velocity is lower than the free',
          'stream velocity.',
          'created by the normal shock wave at transonic speeds.',
          'a turbulent flow around the wing.',
          'caused by suction on the upper wing surface.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A laminar boundary layer is a layer, in which:',
        options: [
          'no velocity components exist, normal to the surface.',
          'the vortices are weak.',
          'the velocity is constant.',
          'the temperature varies constantly.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The maximum acceptable cruising altitude is limited by a minimum acceptable loadfactor because exceeding that altitude:',
        options: [
          'turbulence may induce Mach buffet.',
          'turbulence may exceed the limit load factor.',
          'a sudden necessary bankangle may exceed the limit load factor.',
          'Mach buffet will occur immediately.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Total pressure is:',
        options: [
          'static pressure plus dynamic pressure.',
          'static pressure minus dynamic pressure.',
          '½ rho V²',
          'measured at a small hole in a surface, parallel to the local stream.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 47',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If an aeroplane is accelerated from subsonic to supersonic speeds, the centre of pressure will move:',
        options: [
          'to the mid chord position.',
          'forward.',
          'to a position near the leading edge.',
          'to a position near the trailing edge.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The (subsonic) static pressure:',
        options: [
          'decreases in a flow in a tube when the diameter decreases.',
          'is the total pressure plus the dynamic pressure.',
          'is the pressure in a point at which the velocity has become zero.',
          'increases in a flow in a tube when the diameter decreases.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Vortex generators on the upper side of the wing surface will:',
        options: [
          'decrease the intensity of shock wave induced air separation.',
          'increase the critical Mach Number.',
          'decrease the span wise flow at high Mach Numbers.',
          'increase the magnitude of the shock wave.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The true airspeed (TAS) is:',
        options: [
          'lower than the indicated airspeed (IAS) at ISA conditions and altitudes',
          'below sea level.',
          'higher than the speed of the undisturbed airstream about the aeroplane.',
          'equal to the IAS, multiplied by the air density at sea level.',
          'lower than the speed of the undisturbed airstream about the aeroplane.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 48',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Vortex generators on the upper side of the wing:',
        options: [
          'decrease wave drag.',
          'increase wave drag.',
          'increase critical Mach Number.',
          'decrease critical Mach Number.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The lift- and drag forces, acting on a wing cross section:',
        options: [
          'depend on the pressure distribution about the wing cross section.',
          'are normal to each other at just one angle of attack.',
          'are proportional to each other, independent of angle of attack.',
          'vary linearly with the angle of attack.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The lift force, acting on an aerofoil:',
        options: [
          'is mainly caused by suction on the upperside of the aerofoil.',
          'increases, proportional to the angle of attack until 40 degrees.',
          'is mainly caused by overpressure at the underside of the aerofoil.',
          'is maximum at an angle of attack of 2 degrees.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The aft movement of the centre of pressure during acceleration through the transonic range will:',
        options: [
          'increase the static longitudinal stability.',
          'decrease the longitudinal stability.',
          'increase the static lateral stability.',
          'decrease the static lateral stability.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 49',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In supersonic flight aerofoil pressure distribution is:',
        options: [
          'rectangular.',
          'irregular.',
          'triangular.',
          'the same as in subsonic flight.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The relative thickness of an aerofoil is expressed in:',
        options: [
          '% chord.',
          'camber.',
          'meters.',
          'degrees cross section tail angle.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The aerofoil polar is:',
        options: [
          'a graph of the relation between the lift coefficient and the drag',
          'coefficient.',
          'a graph of the relation between the lift coefficient and the angle of',
          'attack.',
          'the relation between the horizontal and the vertical speed.',
          'a graph, in which the thickness of the wing aerofoil is given as a',
          'function of the chord.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Shock stall is:',
        options: [
          'separation of the boundary layer behind the shock wave.',
          'separation of the flow behind the bow wave.',
          'separation of the flow at high angles of attack and at high Mach',
          'Numbers.',
          'separation of the flow at the trailing edge of the wing at high Mach',
          'Numbers.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 50',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The aspect ratio of the wing:',
        options: [
          'is the ratio between the wing span and the mean geometric chord.',
          'is the ratio between the wing span and the root chord.',
          'is the ratio between the tip chord and the wing span.',
          'is the ratio between chord and root chord.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In the transonic range the aeroplane characteristics are strongly determined by:',
        options: [
          'the Mach Number.',
          'the TAS.',
          'the IAS.',
          'the CAS.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Dihedral of the wing is:',
        options: [
          'the angle between the 0.25 chord line of the wing and the lateral axis.',
          'the angle between the leading edge of the wing and the lateral axis.',
          'the angle between the 0.25 chord line of the wing and the vertical axis.',
          'the angle between the 0.25 chord line of the wing and the horizon.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following flight phenomena can only occur at Mach numbers above the critical Mach number?',
        options: [
          'Mach buffet.',
          'Dutch roll.',
          'Speed instability.',
          'Elevator stall.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 51',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The induced drag:',
        options: [
          'increases as the lift coefficient increases.',
          'increases as the aspect ratio increases.',
          'has no relation to the lift coefficient.',
          'increases as the magnitude of the tip vortices decreases.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following flight phenomena can occur at Mach numbers below the critical Mach number?',
        options: [
          'Dutch roll.',
          'Tuck under.',
          'Mach buffet.',
          'Shock stall.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Flap selection at constant IAS whilst maintaining straight and level flight will increase the:',
        options: [
          'maximum lift coefficient (CLmax) and the drag.',
          'lift coefficient and the drag.',
          'stall speed.',
          'lift and the drag.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The Mach trim system will:',
        options: [
          'adjust the stabilizer, depending on the Mach Number.',
          'keep the Mach Number automatically constant.',
          'pump the fuel from tank to tank, depending on the Mach Number.',
          'adjust the elevator trim tab, depending on the Mach Number.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 52',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If the sum of all the moments in flight is not zero, the aeroplane will rotate about the:',
        options: [
          'centre of gravity.',
          'neutral point of the aeroplane.',
          'aerodynamic centre of the wing.',
          'centre of pressure of the wing.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The Mach trim system will prevent:',
        options: [
          'tuck under.',
          'dutch roll.',
          'buffeting.',
          'shock stall.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Dihedral of the wing:',
        options: [
          'increases the static lateral stability.',
          'is the only way to increase the static lateral stability.',
          'is only positive for aeroplanes with high mounted wings.',
          'decreases the static lateral stability.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When air has passed an expansion wave, the static pressure is:',
        options: [
          'decreased.',
          'decreased or increased, depending on Mach Number.',
          'increased.',
          'unchanged.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 53',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The critical Mach Number of an aeroplane is the free stream Mach Number, which produces the first evidence of:',
        options: [
          'local sonic flow.',
          'buffet.',
          'shock wave.',
          'supersonic flow.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A C.G location beyond the aft limit leads to:',
        options: [
          'an unacceptable low value of the manoeuvre stability (stick force per g,',
          'Fe/g).',
          'a too high pulling stick force during rotation in the take off.',
          'an increasing static longitudinal stability.',
          'a better recovery performance in the spin.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If the elevator trim tab is deflected up, the cockpit trim indicator presents:',
        options: [
          'nose-down.',
          'neutral.',
          'nose-up.',
          'nose-left.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The critical Mach Number of an aeroplane can be increased by:',
        options: [
          'sweep back of the wings.',
          'vortex generators.',
          'control deflection',
          'dihedral of the wings.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 54',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Differential aileron deflection:',
        options: [
          'equals the drag of the right and left aileron.',
          'is required to keep the total lift constant when ailerons are deflected.',
          'increases the CLmax.',
          'is required to achieve the required roll-rate.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In supersonic flight, all disturbances produced by an aeroplane are:',
        options: [
          'in between a conical area, depending on the Mach Number.',
          'outside the conical area depending on the Mach Number.',
          'in front of the aeroplane.',
          'very weak and negligible.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In transonic flight the ailerons will be less effective than in subsonic flight because:',
        options: [
          'aileron deflection only partly affects the pressure distribution around the',
          'wing.',
          'behind the shock wave pressure is lower.',
          'aileron down deflection moves the shock wave forward.',
          'aileron deflection only affects the air in front of the shock wave.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 55',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An example of differential aileron deflection during initiation of left turn is:',
        options: [
          'Left aileron: 5° up',
          'Right aileron: 2° down',
          'Left aileron: 2° up',
          'Right aileron: 5° down',
          'Left aileron: 5° down',
          'Right aileron: 2° up',
          'Left aileron: 2° down',
          'Right aileron: 5° up',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'To be able to predict compressibility effects you have to determine the:',
        options: [
          'Mach Number.',
          'EAS.',
          'TAS.',
          'IAS.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Does the pitch-angle of a constant-speed propeller alter in medium horizontal turbulence?',
        options: [
          'Yes slightly.',
          'Yes strongly.',
          'No.',
          'Yes, but only if the pitch is full-fine.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 56',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When trailing edge flaps are selected down whilst maintaining straight and level flight at constant IAS:',
        options: [
          'the centre of pressure moves aft.',
          'the lift coefficient and the drag coefficient increase.',
          'the stall speed increases.',
          'the total boundary layer becomes laminar.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The formula for the Mach Number is:',
        options: [
          '(a= speed of sound)',
          'M= TAS / a',
          'M= a / TAS',
          'M= TAS*a',
          'M= IAS / a',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Assuming ISA conditions, climbing at a constant Mach Number up to FL 350 the TAS will:',
        options: [
          'decrease.',
          'first increase, then decrease.',
          'increase.',
          'remain constant.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following situations leads to a decreasing stall speed (IAS)?',
        options: [
          'decreasing weight.',
          'increasing altitude.',
          'increasing air density.',
          'increasing load factor.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 57',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The speed of sound is affected by the:',
        options: [
          'temperature of the air.',
          'density of the air.',
          'pressure of the air.',
          'humidity of the air.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Two identical aircraft A and B, with the same mass, are flying steady level co-ordinated 20 degree bank turns. If the TAS of A is 130 kt and that of B is 200 kt:',
        options: [
          'the rate of turn of A is greater than that of B.',
          'the load factor of A is greater than that of B.',
          'the turn radius of A is greater than that of B.',
          'the lift coefficient of A is less than that of B.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements about a constant speed propeller is correct?',
        options: [
          'The blade angle increases with increasing aeroplane speed.',
          'The propeller system keeps the aeroplane speed constant.',
          'The RPM decreases with increasing aeroplane speed.',
          'The selected RPM is kept constant by the manifold pressure.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane is flying through the transonic range. As the Mach Number increases the centre of pressure of the wing will move aft. This movement requires:',
        options: [
          'a pitch up input of the stabilizer.',
          'a stability augmentation system to improve dynamic stability.',
          'much more thrust from the engine.',
          'a higher IAS to compensate the nose down effect.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 58',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The difference between IAS and TAS will:',
        options: [
          'decrease with decreasing altitude.',
          'increase with decreasing temperature.',
          'increase with increasing air density.',
          'decrease with increasing speed.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The additional increase of drag at Mach Numbers above the critical Mach Number is due to:',
        options: [
          'wave drag.',
          'increased angle of attack.',
          'increased interference drag.',
          'increased skin friction.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Air passes a normal shock wave. Which of the following statements is correct?',
        options: [
          'The static temperature increases.',
          'The static pressure decreases.',
          'The static temperature decreases.',
          'The velocity increases.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Increase of wing loading will:',
        options: [
          'increase the stall speeds.',
          'decrease the minimum gliding angle.',
          'increase CLmax.',
          'decrease take off speeds.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 59',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane performs a right turn, the slip indicator is left of neutral. One way to co-ordinate the turn is to apply:',
        options: [
          'more left rudder.',
          'more right rudder.',
          'less right bank.',
          'a higher turn-rate.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The bow wave will first appear at:',
        options: [
          'a Mach number just above M = 1',
          'a Mach number just below M = 1',
          'Mach 1',
          'the critical Mach number',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A light twin is in a turn at 20 degrees bank and 150 kt TAS. A more heavy aeroplane at the same bank and the same speed will:',
        options: [
          'turn at the same turn radius.',
          'turn at a bigger turn radius.',
          'turn at a smaller turn radius.',
          'turn at a higher turn rate.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Two methods to increase the critical Mach Number are:',
        options: [
          'thin aerofoils and sweep back of the wing.',
          'thin aerofoils and dihedral of the wing.',
          'positive cambering of the aerofoil and sweep back of the wing.',
          'thick aerofoils and dihedral of the wing.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 60',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A commercial jet aeroplane is performing a straight descent at a constant Mach number with constant mass. The operational speed limit that may be exceeded is:',
        options: [
          'VMO.',
          'VNE.',
          'VD.',
          'MMO.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct about an aeroplane, that has experienced a left engine failure and continues afterwards in straight and level cruise flight with wings level ?',
        options: [
          'turn indicator neutral, slip indicator neutral.',
          'turn indicator neutral, slip indicator left of neutral.',
          'turn indicator left of neutral, slip indicator left of neutral.',
          'turn indicator left of neutral, slip indicator neutral.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The bank angle in a rate-one turn depends on:',
        options: [
          'TAS.',
          'weight.',
          'load factor.',
          'wind.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 61',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The relationship between the stall speed VS and VA (EAS) for a large transport aeroplane can be expressed in the following formula:',
        options: [
          '(SQRT= square root)',
          'VA >= VS SQRT(2.5)',
          'VA > VS SQRT(2.5)',
          'VA <= VS SQRT(2.5)',
          'VA < VS SQRT(2.5)',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement about induced drag and tip vortices is correct?',
        options: [
          'The flow direction at the upper side of the wing has a component in',
          'wing root direction, the flow at the underside of the wing in wing tip',
          'direction.',
          'Tip vortices can be diminished by vortex generators.',
          'The flow direction at the upper and under side of the wing, both deviate',
          'in wing tip direction.',
          'The wing tip vortices and the induced drag decrease at increasing angle',
          'of attack.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'By what percentage does VA (EAS) alter when the aeroplane\'s weight decreases by 19%?',
        options: [
          '10% lower.',
          '4.36% lower.',
          'no change',
          '19% lower.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 62',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which load factor determines VA?',
        options: [
          'manoeuvring limit load factor.',
          'manoeuvring ultimate load factor.',
          'gust load factor at 66 ft/sec gust.',
          'manoeuvring flap limit load factor.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'VA is:',
        options: [
          'the maximum speed at which maximum elevator deflection up is',
          'allowed.',
          'the maximum speed at which rolls are allowed.',
          'the speed at which a heavy transport aeroplane should fly in turbulence.',
          'the speed that should not be exceeded in the climb.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Compared with stalling airspeed (VS) in a given configuration, the airspeed at which stick shaker will be triggered is:',
        options: [
          'greater than VS.',
          '1.20 VS.',
          '1.30 VS.',
          '1.12 VS.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'VMO:',
        options: [
          'should be not greater than VC.',
          'should be chosen in between VC and VD.',
          'is equal to the design speed for maximum gust intensity.',
          'should not be less than VD.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 63',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What can happen to the aeroplane structure flying at a speed just exceeding VA ?',
        options: [
          'It may suffer permanent deformation if the elevator is fully deflected',
          'upwards',
          'It may break if the elevator is fully deflected upwards.',
          'It may suffer permanent deformation because the flight is performed at',
          'too large dynamic pressure.',
          'It will collapse if a turn is made.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The term angle of attack in a two dimensional flow is defined as:',
        options: [
          'the angle between the wing chord line and the direction of the relative',
          'wind/airflow.',
          'the angle between the aeroplane climb path and the horizon.',
          'the angle formed by the longitudinal axis of the aeroplane and the chord',
          'line of the wing',
          'the angle for maximum lift/drag ratio',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the limit load factor of a large transport aeroplane in the manoeuvring diagram?',
        options: [
          '2.5',
          '1.5',
          '3.75',
          '6',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 64',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The terms "q" and "S" in the lift formula are:',
        options: [
          'dynamic pressure and the area of the wing',
          'square root of surface and wing loading',
          'static pressure and wing surface area',
          'static pressure and dynamic pressure',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The critical angle of attack:',
        options: [
          'remains unchanged regardless of gross weight',
          'increases if the CG is moved forward',
          'decreases if the CG is moved aft',
          'changes with an increase in gross weight',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Comparing the lift coefficient and drag coefficient at normal angle of attack:',
        options: [
          'CL is much greater than CD',
          'CL has approximately the same value as CD',
          'CL is lower than CD',
          'CL is much lower than CD',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct in respect of Cl and angle of attack?',
        options: [
          'For a symmetrical aerofoil, if the angle of attack is zero, Cl is zero.',
          'For a symmetrical aerofoil, if the angle of attack is zero, Cl is not zero.',
          'For an asymmetrical aerofoil, if the angle of attack is zero, Cl is zero.',
          'For an asymmetrical aerofoil with positive camber, if the angle of attack',
          'is greater than zero, Cl is zero.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 65',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The polar curve of an aerofoil is a graphic relation between:',
        options: [
          'CL and CD',
          'TAS and stall speed',
          'Angle of attack and CL',
          'CD and angle of attack',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The Mean Aerodynamic Chord (MAC) for a given wing of any planform is',
        options: [
          'the chord of a rectangular wing with same moment and lift',
          'the chord of a large rectangular wing',
          'the average chord of the actual aeroplane',
          'the wing area divided by the wing span',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The span-wise flow is caused by the difference between the air pressure on top and beneath the wing and its direction of movement goes from:',
        options: [
          'beneath to the top of the wing via the wing tip',
          'the top to beneath the wing via the wing\'s trailing edge',
          'beneath to the top of the wing via the trailing edge',
          'the top to beneath the wing via the leading edge',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Induced drag may be reduced by:',
        options: [
          'an increase in aspect ratio',
          'an increase in the taper ratio of the wing',
          'a decrease of the aspect ratio',
          'the use of a wing tip with a much thinner aerofoil',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 66',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The relationship between induced drag and the aspect ratio is:',
        options: [
          'a decrease in the aspect ratio increases the induced drag',
          'there is no relationship',
          'induced drag = 1.3 aspect ratio value',
          'an increase in the aspect ratio increases the induced drag',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A high aspect ratio wing produces:',
        options: [
          'a decrease in induced drag',
          'less sensitivity to gust effects',
          'a decrease in stall speed',
          'an increase in induced drag',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the effect on induced drag of mass and speed changes? (all other factors of importance remaining constant) Decreases with increasing speed and decreasing mass.',
        options: [
          'Decreases with decreasing speed and decreasing mass.',
          'Increases with increasing speed and decreasing mass.',
          'Increases with increasing speed and  increasing mass.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What will happen in ground effect?',
        options: [
          'The induced angle of attack and induced drag decrease.',
          'The wing downwash on the tail surfaces increases.',
          'The wing tip vortices increase in strength.',
          'The thrust required will increase significantly.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 67',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Floating due to ground effect during an approach to land will occur:',
        options: [
          'when the height is less than halve of the length of the wing span above',
          'the surface',
          'when the height is less than twice the length of the wing span above the',
          'surface',
          'when a higher than normal angle of attack is used',
          'at a speed approaching the stall',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct about the laminar and turbulent boundary layer:',
        options: [
          'friction drag is lower in the laminar layer',
          'friction drag will be equal in both types of layers',
          'friction drag is lower in the turbulent layer',
          'separation point will occur earlier in the turbulent layer',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'After the transition point between the laminar and turbulent boundary layer',
        options: [
          'the mean speed and friction drag increases',
          'the boundary layer gets thinner and the speed increases',
          'the mean speed increases and the friction drag decreases',
          'the boundary layer gets thicker and the speed decreases',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The stall speed:',
        options: [
          'increases with an increased weight',
          'decreases with an increased weight',
          'does not depend on weight',
          'increases with the length of the wingspan',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 68',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When a pilot makes a turn in horizontal flight, the stall speed:',
        options: [
          'increases with the square root of load factor',
          'increases with flap extension',
          'decreases with increasing bank angle',
          'increases with the load factor squared',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The stall speed in a 60° banked turn increases by the following factor:',
        options: [
          '1.41',
          '1.07',
          '1.30',
          '2.00',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The trailing edge flaps when extended:',
        options: [
          'degrade the best angle of glide',
          'increase the zero lift angle of attack',
          'significantly increase the angle of attack for maximum lift',
          'significantly lower the drag',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When trailing edge flaps are extended in level flight, the change in pitching moment, ignoring any effects on the tailplane, will be:',
        options: [
          'nose down.',
          'nose up.',
          'zero.',
          'dependent on cg location.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 69',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Deflection of leading edge flaps will:',
        options: [
          'increase critical angle of attack.',
          'decrease CLmax.',
          'decrease drag.',
          'not affect critical angle of attack.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Slat extension will:',
        options: [
          'increase the critical angle of attack.',
          'reduce tip vortices.',
          'create gaps between leading edge and engine nacelles.',
          'decrease the energy in the boundary layer on the upperside of the wing.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Spoiler deflection causes:',
        options: [
          'an increase in drag and decrease in lift',
          'an increase in lift and drag',
          'an increase in lift only',
          'decrease in lift and drag',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane performs a continuous descent with 160 kts IAS and 1000 feet/min vertical speed. In this condition:',
        options: [
          'weight is greater than lift',
          'lift is equal to weight',
          'lift is less than drag',
          'drag is less than the combined forces that move the aeroplane forward',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 70',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'One disadvantage of wing sweep is the stalling characteristics. At the stall:',
        options: [
          'tip stall will occur first, which produces a pitch-up moment.',
          'wing root stall will occur first, which produces a rolling moment',
          'tip stall will occur first, which produces a nose-down moment',
          'leading edge stall will occur first, which produces a nose-down moment',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The Mach number:',
        options: [
          'is the ratio between the TAS of the aeroplane and the local speed of',
          'sound.',
          'is the ratio between the TAS of the aeroplane and the speed of sound at',
          'sea level.',
          'is the ratio between the IAS of the aeroplane and the local speed of',
          'sound.',
          'increases at a given TAS, when the temperature rises.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What factors determine the distance travelled over the ground of an aeroplane in a glide ?',
        options: [
          'The wind and the lift/drag ratio, which changes with angle of attack',
          'The wind and the aeroplane\'s mass',
          'The wind and CLmax',
          'The wind and weight together with power loading, which is the ratio of',
          'power output to the weight',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 71',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A normal shock wave:',
        options: [
          'can occur at different points on the aeroplane in transonic flight.',
          'is a discontinuity plane in an airflow, in which the temperature drops',
          'suddenly.',
          'is a discontinuity plane in an airflow, in which the pressure drops',
          'suddenly.',
          'is a discontinuity plane in an airflow, which is always normal to the',
          'surface.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The following unit of measurement: kgm/s² is expressed in the SI-system as:',
        options: [
          'Newton',
          'Pascal',
          'Joule',
          'Watt',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Excluding constants, the coefficient of induced drag (CDi) is the ratio of:',
        options: [
          'CL² and AR (aspect ratio)',
          'CL and CD',
          'CL and b (wing span)',
          'CL²and S (wing surface)',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'One important advantage the turbulent boundary layer has over the laminar layer is that:',
        options: [
          'it has less tendency to separate from the surface',
          'it is thinner',
          'skin friction drag is less',
          'energy is less',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 72',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which kind of \'\'tab\'\' is commonly used in case of manual reversion of fully powered flight controls ?',
        options: [
          'Servo tab',
          'Spring tab',
          'Balance tab',
          'Anti-balance tab',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'High Aspect Ratio, as compared with low Aspect Ratio, has the effect of:',
        options: [
          'Decreasing induced drag and critical angle of attack',
          'Increasing lift and critical angle of attack',
          'Increasing lift and drag',
          'Increasing induced drag and decreasing critical angle of attack',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What wing shape or wing characteristic is the least sensitive to turbulence:',
        options: [
          'swept wings',
          'straight wings',
          'wing dihedral',
          'elliptical wing',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '"A line connecting the leading- and trailing edge midway between the upper and lower surface of a aerofoil". This definition is applicable for:',
        options: [
          'the camber line',
          'the chord line',
          'the mean aerodynamic chord line',
          'the upper camber line',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 73',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'One advantage of a movable-stabilizer system compared with an elevator trim system is that:',
        options: [
          'it is a more effective means of trimming',
          'the complete system (structure and control mechanism) weighs less',
          'it leads to greater stability in flight',
          'the system\'s complexity is reduced',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane has a stall speed of 78 KCAS at its gross weight of 6850 Ibs.',
        options: [
          'What is the stall speed when the weight is 5000 Ibs ?',
          '67 KCAS',
          '91 KCAS',
          '78 KCAS',
          '57 KCAS',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct about a spring tab ?',
        options: [
          'At high IAS it behaves like a servo tab',
          'At low IAS it behaves like a servo tab',
          'At high IAS it behaves like a fixed extension of the elevator',
          'Its main purpose is to increase stick force per g',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'How is adverse yaw compensated for during entry into and roll out from a turn ?',
        options: [
          'Differential aileron deflection',
          'Horn-balanced controls',
          'Anti-balanced rudder control',
          'Servo tabs',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 74',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What increases the stalling angle of attack ? Use of:',
        options: [
          'slats',
          'flaps',
          'spoilers',
          'fuselage mounted speed-brakes',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the unit of measurement for power ?',
        options: [
          'Nm/s',
          'kgm/s²',
          'Pa/m²',
          'N/m',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The use of a slot in the leading edge of the wing enables the aeroplane to fly at a slower speed because:',
        options: [
          'it delays the stall to a higher angle of attack',
          'the laminar part of the boundary layer gets thicker',
          'it decelerates the upper surface boundary layer air',
          'it changes the camber of the wing',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The angle of attack of a fixed pitch propeller blade increases when:',
        options: [
          'RPM increases and forward velocity decreases',
          'velocity and RPM increase',
          'forward velocity increases and RPM decreasing',
          'velocity and RPM decrease',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 75',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the approximate value of the lift of an aeroplane at a gross weight of 50 000 N, in a horizontal coordinated 45 degrees banked turn ?',
        options: [
          '70 000 N',
          '60 000 N',
          '50 000 N',
          '80 000 N',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Load factor is:',
        options: [
          'Lift/Weight',
          'Weight/Lift',
          '1/Bank angle',
          'Wing loading',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which has the effect of increasing load factor ? (all other relevant factors',
        options: [
          'being constant)',
          'Vertical gusts',
          'Increased aeroplane mass',
          'Increased air density',
          'Rearward CG location',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In twin-engine aeroplanes with right turning propellers the left engine is the critical motor.',
        options: [
          'the left engine produces a higher yaw moment if the right engine fails',
          'than vice versa.',
          'the \'minimum control speed\' is determined by the failure of the right',
          'engine.',
          'the right engine is the critical motor.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 76',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The continuity equation states: If the area of a tube is increasing, the speed of the subsonic and incompressible flow inside is decreasing.',
        options: [
          'not changing.',
          'increasing.',
          'sonic.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If the continuity equation is applicable, what will happen to the air density',
        options: [
          '(rho) if the cross sectional area of a tube changes ? (low speed, subsonic and',
          'incompressible flow)',
          'rho1 = rho2',
          'rho1 < rho2',
          'rho1 > rho2',
          'The density depends on the change of the tube area.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The critical Mach number for an aerofoil equals the free stream airfoil Mach number at which:',
        options: [
          'sonic speed (M=1) is reached at a certain point on the upper side of the',
          'aerofoil.',
          'the maximum operating temperature is reached.',
          'a shock-wave appears on the upper surface.',
          'a "supersonic bell" appears on the upper surface.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 77',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Bernoulli\'s equation can be written as:',
        options: [
          '(pt = total pressure, ps = static pressure, q = dynamic pressure)',
          'pt - q = ps',
          'pt = ps - q',
          'pt + ps = q',
          'pt = q - ps',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which kind of boundary layer has the strongest change in velocity close to the surface?',
        options: [
          'Turbulent boundary layer',
          'Laminar boundary layer',
          'No difference',
          'Transition boundary layer',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-002.jpg Which one of the bodies in motion (all bodies have the same cross section area) will have lowest drag?',
        options: [
          'Body c',
          'Body a',
          'Body b',
          'Body d',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 78',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Increasing dynamic pressure will have the following effect on the drag of an aeroplane (all other factors of importance remaining constant):',
        options: [
          'at speeds greater than the minimum drag speed, drag increases.',
          'none',
          'drag decreases across the whole speed range.',
          'drag increases across the whole speed range.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Increasing air density will have the following effect on the drag of a body in an airstream (angle of attack and TAS are constant):',
        options: [
          'the drag increases.',
          'this has no effect.',
          'the drag decreases.',
          'the drag is only affected by the ground speed.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following aeroplane parts affect induced drag most?',
        options: [
          'Wing tip',
          'Engine cowling',
          'Wing root junction',
          'Landing gear',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Winglets decrease the induced drag.',
        options: [
          'decrease the static lateral stability.',
          'increase the manoeuvrability.',
          'create an elliptical lift distribution.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 79',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Interference drag is the result of:',
        options: [
          'aerodynamic interaction between aeroplane parts (e.g. wing/fuselage).',
          'downwash behind the wing.',
          'separation of the induced vortex.',
          'the addition of induced and parasite drag',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-003.jpg Which line represents the total drag line of an aeroplane?',
        options: [
          'Line c',
          'Line a',
          'Line b',
          'Line d',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-004.jpg The diagram shows the parameter X versus TAS. If a horizontal flight is considered the axis X shows the induced drag.',
        options: [
          'the total drag.',
          'the lift force.',
          'the parasite drag.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 80',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-005.jpg How are the speeds (shown in the figure) at point 1 and point 2 related to the relative wind/airflow V?',
        options: [
          'V1 = 0 and V2 > V',
          'V1 < V2 and V2 < V',
          'V1 = 0 and V2 = V',
          'V1 > V2 and V2 < V',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Consider an aerofoil with a certain camber and a positive angle of attack. At which location will the highest flow velocities occur ?',
        options: [
          'Upper side',
          'Lower side',
          'In front of the stagnation point',
          'In the stagnation point',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The forces of lift and drag on an aerofoil are, respectively, normal and parallel to the:',
        options: [
          'relative wind/airflow.',
          'chord line.',
          'longitudinal axis.',
          'horizon.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 81',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If an aeroplane flies in the ground effect the lift is increased and the drag is decreased.',
        options: [
          'the effective angle of attack is decreased.',
          'the induced angle of attack is increased.',
          'drag and lift are reduced.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Ground effect has the following influence on the landing distance:',
        options: [
          'increases.',
          'decreases.',
          'does not change.',
          'increases, only if the landing flaps are fully extended.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane performs a straight and level horizontal flight at the same angle',
        options: [
          'of attack at two different altitudes. (all other factors of importance being',
          'constant, assume ISA conditions and no compressibility effects)',
          'the TAS at the higher altitude is higher',
          'the TAS at both altitudes is the same',
          'the TAS at the higher altitude cannot be determined',
          'the TAS at the higher altitude is lower',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-006.jpg Which point shown in the figure corresponds with CL for minimum horizontal flight speed?',
        options: [
          'Point d',
          'Point a',
          'Point b',
          'Point c',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 82',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-007.jpg Which point marks the value for minimum sink rate (assume zero thrust) ?',
        options: [
          'Point 3',
          'Point 1',
          'Point 2',
          'Point 4',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-008.jpg Which point in the diagram corresponds to the minimum (zero thrust) glide angle ?',
        options: [
          'Point 2',
          'Point 1',
          'Point 3',
          'Point 4',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-009.jpg Which point in the diagram gives the lowest speed in horizontal flight?',
        options: [
          'Point 4',
          'Point 1',
          'Point 2',
          'Point 3',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'At higher altitudes, the stall speed (IAS):',
        options: [
          'increases',
          'decreases',
          'remains the same',
          'decreases until the tropopause',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 83',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the correct relationship between the true airspeed for (i) minimum sink rate and (ii) minimum glide angle, at a given altitude?',
        options: [
          '(i) is less than (ii)',
          '(i) is equal to (ii)',
          '(i) is greater than (ii)',
          '(i) can be greater than or less than (ii) depending on the type of',
          'aeroplane',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If you decrease the propeller pitch during a glide with idle-power at constant IAS the lift to drag ratio will decrease and the rate of descent will increase.',
        options: [
          'increase and the rate of descent will increase.',
          'decrease and the rate of descent will decrease.',
          'increase and the rate of descent will decrease.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If you increase the propeller pitch during a glide with idle-power at constant IAS the lift to drag ratio will increase and the rate of descent will decrease.',
        options: [
          'increase and the rate of descent will increase.',
          'decrease and the rate of descent will decrease.',
          'decrease and the rate of descent will increase.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 84',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct?',
        options: [
          'The lift to drag ratio determines the',
          'horizontal glide distance from a given altitude at zero wind and zero',
          'thrust.',
          'maximum rate of climb.',
          'endurance speed.',
          'horizontal distance in the climb up to a given altitude.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-010.jpg Which type of flap is shown in the picture?',
        options: [
          'Fowler flap',
          'Double slotted flap',
          'Plain flap',
          'Split flap',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-011.jpg Which type of flap is shown in the picture?',
        options: [
          'Split flap',
          'Single slotted flap',
          'Fowler flap',
          'Plain flap',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 85',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-012.jpg',
        options: [
          'The high lift device shown in the figure is a',
          'Slat',
          'Fowler flap',
          'Slotted flap',
          'Krueger flap',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: '081-013.jpg',
        options: [
          'The high lift device shown in the figure below is a',
          'Krueger flap',
          'Fowler flap',
          'Slotted flap',
          'Slot or slat',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A plain flap will increase CLmax by increasing the camber of the aerofoil.',
        options: [
          'increasing angle of attack.',
          'boundary layer control.',
          'centre of lift movement.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane is descending at a constant Mach number from FL 350. What is the effect on true airspeed ?',
        options: [
          'It increases as temperature increases',
          'It decreases as pressure increases',
          'It decreases as altitude decreases',
          'It remains constant',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 86',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'During the retraction of the flaps at a constant angle of attack the aeroplane starts to (all other factors of importance being constant) sink suddenly.',
        options: [
          'bank.',
          'climb.',
          'yaw.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A jet aeroplane is cruising at high altitude with a Mach-number, that provides a buffet margin of 0.3g incremental. In order to increase the buffet margin to 0.4g incremental the pilot must:',
        options: [
          'fly at a lower altitude and the same Mach-number',
          'extend the flaps to the first selection',
          'fly at a higher Mach-number',
          'fly at a larger angle of attack',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'During the extension of the flaps at a constant angle of attack the aeroplane starts to (all other factors of importance being constant) climb.',
        options: [
          'bank.',
          'sink suddenly.',
          'yaw.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 87',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'For an aeroplane with one fixed value of VA the following applies. VA is:',
        options: [
          'the speed at which the aeroplane stalls at the manoeuvring limit load',
          'factor at MTOW.',
          'the maximum speed in smooth air',
          'the speed at which unrestricted application of elevator control can be',
          'used, without exceeding the maximum manoeuvring limit load factor',
          'just another symbol for the rough air speed',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Compared with the clean configuration, the angle of attack at CLmax with trailing edge flaps extended is:',
        options: [
          'smaller.',
          'larger.',
          'unchanged.',
          'smaller or larger depending on the degree of flap extension.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A slat will prolongs the stall to a higher angle of attack.',
        options: [
          'increase the camber of the aerofoil and divert the flow around the sharp',
          'leading edge.',
          'increase the lift by increasing the wing area and the camber of the aft',
          'portion of the wing.',
          'provide a boundary layer suction on the upper surface of the wing.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 88',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'One method to compensate adverse yaw is:',
        options: [
          'a differential aileron.',
          'a balance tab.',
          'an anti-balance tab.',
          'a balance panel.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Flaperons are controls which combine the function of:',
        options: [
          'ailerons and flaps.',
          'ailerons and elevator.',
          'flaps and speed brakes.',
          'flaps and elevator.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Critical Mach number is the free stream Mach number at which:',
        options: [
          'local supersonic flow first exists on any part of the aeroplane.',
          'there is subsonic flow over all parts of the aeroplane.',
          'there is supersonic flow over all parts of the aeroplane.',
          'the aeroplane has zero buffet margin.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The sensor of a stall warning system can be activated by a change in the location of the stagnation point.',
        options: [
          'centre of lift.',
          'transition region.',
          'centre of gravity.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 89',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which aeroplane design has the highest probability of a super stall?',
        options: [
          'A T-tail.',
          'A canard wing.',
          'Swept wings.',
          'A low horizontal tail.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The pitch up effect of an aeroplane with swept wing in a stall is due to the wing tip stalling first.',
        options: [
          'aft movement of the centre of gravity.',
          'forward movement of the centre of gravity.',
          'wing root stalling first.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The Mach-trim function is installed on most commercial jets in order to minimize the adverse effects of:',
        options: [
          'changes in the position of centre of pressure',
          'increased drag due to shock wave formation',
          'uncontrolled changes in stabilizer setting',
          'compressibility effects on the stabilizer',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A jet aeroplane equipped with inboard and outboard ailerons is cruising at its normal cruise Mach number. In this case only the inboard ailerons are active.',
        options: [
          'only the outboard aileron are active.',
          'the inboard and outboard ailerons are active.',
          'only the spoilers will be active, not the ailerons.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 90',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When comparing a rectangular wing and a swept back wing of the same wing area and wing loading, the swept back wing has the advantage of:',
        options: [
          'Higher critical Mach number',
          'Greater strength',
          'Increased longitudinal stability',
          'Lower stalling speed',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the effect of an aft shift of the centre of gravity on (1) static longitudinal stability and (2) the required control deflection for a given pitch change?',
        options: [
          '(1) reduces     (2) reduces',
          '(1) increases  (2) reduces',
          '(1) increases  (2) increases',
          '(1) reduces    (2) increases',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement about a primary control surface controlled by a servo tab, is correct ?',
        options: [
          'The position is undetermined during taxiing, in particular with tailwind.',
          'The servo tab can also be used as a balance tab.',
          'The control effectiveness of the primary surface is increased by servo',
          'tab deflection.',
          'Due to the effectiveness of the servo tab the control surface area can be',
          'smaller.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 91',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct about a normal shock wave ?',
        options: [
          'The airflow changes from supersonic to subsonic',
          'The airflow changes direction',
          'The airflow changes from subsonic to supersonic',
          'The airflow expands when passing the aerofoil',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Assuming an initial condition in straight and level flight with CL=1. What will be the new value of CL after the value of the speed has doubled?',
        options: [
          '0.25.',
          '0.50.',
          '1.00.',
          '2.00.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane is provided with spoilers and in- and outboard ailerons. Roll control during cruise is provided by:',
        options: [
          'inboard ailerons and roll-spoilers.',
          'outboard ailerons and roll-spoilers.',
          'inboard and outboard ailerons.',
          'outboard ailerons only.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 92',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The airload on the horizontal tailplane (tailload) of an aeroplane in straight and level flight:',
        options: [
          'is in general directed downwards and will become less negative when',
          'the c.g. moves aft.',
          'is in general directed upwards and will increase when c.g. is moved',
          'forward.',
          'will in principle be zero on transport aeroplanes without an electronic',
          'flight control system (Fly-by-Wire) due to the trim system.',
          'is in general directed downwards and will always become less negative',
          'in a linear fashion with increasing airspeed.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The aerodynamic contribution to the static longitudinal stability of the nacelles of aft fuselage mounted engines is:',
        options: [
          'positive.',
          'negative.',
          'zero.',
          'maximum during cruise.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Examples of aerodynamic balancing of control surfaces are:',
        options: [
          'seal between wing\'s trailing edge and leading edge of a control surface,',
          'horn balance',
          'upper and lower rudder, seal between wing\'s trailing edge and leading',
          'edge of a control surface',
          'weight in the nose of the control surface, horn balance',
          'Fowler flaps, upper and lower rudder',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 93',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In straight and level flight, as speed is increased:',
        options: [
          'the elevator is deflected further downwards and the trim tab further',
          'upwards.',
          'both elevator and trim tab are deflected further downwards.',
          'the elevator and trim tab do not move.',
          'the elevator is deflected further upwards and the trim tab further',
          'downwards.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements concerning control is correct?',
        options: [
          'In a differential aileron control system the control surfaces have a larger',
          'upward than downward maximum deflection.',
          'On some aeroplanes, the servo tab also serves as a trim tab.',
          'Hydraulically powered control surfaces do not need mass balancing.',
          'In general the maximum downward elevator deflection is larger than',
          'upward.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When are outboard ailerons (if present) de-activated ?',
        options: [
          'Flaps (and slats) retracted or speed above a certain value.',
          'Flaps (and/or slats) extended or speed below a certain value..',
          'Landing gear retracted.',
          'Landing gear extended.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 94',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In what flight phase are the outboard ailerons (if present) not operated ?',
        options: [
          'During cruise flight.',
          'During take-off, until lift-off.',
          'In the approach phase, before landing.',
          'During a landing with strong and gusty crosswind to avoid',
          'overcontrolling the aeroplane.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Increase of the delta load factor due to a given vertical upgust at a given EAS occurs when:',
        options: [
          'the slope of the CL-alpha graph becomes steeper.',
          'the weight increases.',
          'the altitude increases.',
          'the wing loading increases.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'For shallow climb angles the following formula can be used:',
        options: [
          '(gamma = climb angle)',
          'sin gamma = T/W - CD/CL.',
          'sin gamma = W/T - CD/CL.',
          'sin gamma = W/T - CL/CD.',
          'sin gamma = T/W - CL/CD.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Positive static lateral stability is the tendency of an aeroplane to:',
        options: [
          'roll to the left in the case of a positive sideslip angle (aeroplane nose to',
          'the left).',
          'roll to the right in the case of a positive sideslip angle (aeroplane nose',
          'to the right).',
          'roll to the right in a right turn.',
          'roll to the left in a right turn.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 95',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Static lateral stability should not be too large, because:',
        options: [
          'too much aileron deflection would be required in a crosswind landing.',
          'too much rudder deflection would be required in a crosswind landing.',
          'constant aileron deflection would be required during cruise in case of',
          'crosswind.',
          'the roll trim sensitivity would increase sharply.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Static lateral stability should not be too small, because the aeroplane would show too strong a tendency to spiral dive.',
        options: [
          'after a disturbance around the longitudinal axis the aeroplane would',
          'show too strong a tendency to return to the original attitude.',
          'the stick force per g would become unacceptably small.',
          'the aeroplane would show too strong a tendency for Dutch Roll.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What should be usually done to perform a landing with the stabilizer jammed in the cruise flight position ?',
        options: [
          'choose a higher landing speed than normal and/or use a lower',
          'flapsetting for landing.',
          'choose a lower landing speed than normal.',
          'if possible, relocate as many passengers as possible to the front of the',
          'cabin.',
          'use the Mach trimmer until after landing.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 96',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In general transport aeroplanes with power assisted flight controls are fitted with an adjustable stabilizer instead of trim tabs on the elevator. This is because:',
        options: [
          'effectiveness of trim tabs is insufficient for those aeroplanes',
          'the pilot does not feel the stick forces at all',
          'mechanical adjustment of trim tabs creates too many problems',
          'trim tab deflection increases Mcrit',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'At a constant angle of attack, which of the following factors will lead to an increase of ground distance during a glide and with zero thrust ?',
        options: [
          'Tailwind',
          'Headwind',
          'Increase of aeroplane mass',
          'Decrease of aeroplane mass',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following parameters can be read from the parabolic polar diagram of an aeroplane?',
        options: [
          'The minimum glide angle and the parasite drag coefficient.',
          'The minimum rate of descent and the induced drag.',
          'The induced drag and the parasite drag.',
          'The aspect ratio of the wing and the induced drag coefficient.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 97',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane transitions from steady straight and level flight into a horizontal co-ordinated turn with a load factor of 2, the speed remains constant and the:',
        options: [
          'induced drag increases by a factor of 4.',
          'angle of attack increases by a factor of 1/4.',
          'lift increases by a factor of 4.',
          'total drag increases by a factor of 4.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Bernoulli\'s law states:',
        options: [
          '(note:',
          'rho is the mean sea level density under ISA conditions;',
          'pstat is static pressure;',
          'pdyn is dynamic pressure;',
          'ptot is total pressure)',
          'pstat + ½rhoV²= constant.',
          'pstat + ½rhoTAS² = constant.',
          'pdyn+ ½rhoV²= constant.',
          'ptot+ ½rhoV²= pstat.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The wing of an aeroplane will never stall at low subsonic speeds as long as....',
        options: [
          'the angle of attack is smaller than the value at which the stall occurs.',
          'the IAS exceeds the power-on stall speed.',
          'the CAS exceeds the power-on stall speed.',
          'there is a nose-down attitude.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 98',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The induced drag coefficient, CDi is proportional with:',
        options: [
          'CL²',
          'CL',
          'square root (CL)',
          'CLmax',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The stall speed increases, when: (all other factors of importance being constant) pulling out of a dive.',
        options: [
          'weight decreases.',
          'minor altitude changes occur e.g. 0-10.000 ft.',
          'spoilers are retracted.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When power assisted controls are used for pitch control:',
        options: [
          'a part of the aerodynamic forces is still felt on the column.',
          'trimming is superfluous.',
          'aerodynamic balancing of the control surfaces is meaningless.',
          'they only function in combination with an elevator trim tab.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Sensitivity for spiral dive will occur when:',
        options: [
          'the static directional stability is positive and the static lateral stability is',
          'relatively weak.',
          'the static directional stability is negative and the static lateral stability is',
          'positive.',
          'the static lateral and directional stability are both negative.',
          'the dutch roll tendency is too strongly suppressed by the yaw damper.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 99',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which part of an aeroplane provides the greatest positive contribution to the static longitudinal stability ?',
        options: [
          'The horizontal tailplane.',
          'The engine.',
          'The fuselage.',
          'The wing.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement about stick force per g is correct?',
        options: [
          'The stick force per g must have both an upper and lower limit in order to',
          'ensure acceptable control characteristics.',
          'The stick force per g increases, when centre of gravity is moved aft.',
          'The stick force per g can only be corrected by means of electronic',
          'devices (stability augmentation) in case of an unacceptable value.',
          'If the slope of the Fe-n line becomes negative, generally speaking this is',
          'not a problem for control of an aeroplane.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Why is VMCG determined with the nosewheel steering disconnected?',
        options: [
          'Because the value of VMCG must also be applicable on wet and/or',
          'slippery runways.',
          'Because the nosewheel steering could become inoperative after an',
          'engine has failed.',
          'Because it must be possible to abort the take-off even after the',
          'nosewheel has already been lifted off the ground.',
          'Because nosewheel steering has no effect on the value of VMCG.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 100',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'By what approximate percentage will the stall speed increase in a horizontal coordinated turn with a bank angle of 45° ?',
        options: [
          '19%',
          '31%',
          '41%',
          '52%',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane has a stalling speed of 100 kt in a steady level flight. When the aeroplane is flying a level turn with a load factor of 1.5, the stalling speed is:',
        options: [
          '122 kt.',
          '141 kt.',
          '82 kt.',
          '150 kt.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following wing planforms gives the highest local lift coefficient at the wing root ?',
        options: [
          'Rectangular.',
          'Elliptical.',
          'Tapered.',
          'Positive angle of sweep.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Examples of aerodynamic balancing of control surfaces are:',
        options: [
          'servo tab, spring tab, seal between the wing trailing edge and the',
          'leading edge of control surface.',
          'balance tab, horn balance, and mass balance.',
          'mass in the nose of the control surface, horn balance and mass balance.',
          'spring tab, servo tab, and power assisted control.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 101',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In straight and level flight, as speed is reduced:',
        options: [
          'the elevator is deflected further upwards and the trim tab further',
          'downwards.',
          'both elevator and trim tab are deflected further upwards.',
          'the elevator and trim tab do not move.',
          'the elevator is deflected further downwards and the trim tab further',
          'upwards.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct about the gust load on an aeroplane (IAS and all other factors of importance remaining constant) ?',
        options: [
          '1. the gust load increases, when the weight decreases.',
          '2. the gust load increases, when the altitude increases.',
          '1 is correct and 2 is incorrect.',
          '1 and 2 are correct.',
          '1 and 2 are incorrect.',
          '1 is incorrect and 2 is correct.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The angle of attack of a wing profile is defined as the angle between:',
        options: [
          'The undisturbed airflow and the chordline.',
          'The local airflow and the mean camberline.',
          'The local airflow and the chordline.',
          'The undisturbed airflow and the mean camberline.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 102',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An advantage of locating the engines at the rear of the fuselage, in comparison to a location beneath the wing, is:',
        options: [
          'less influence of thrust changes on longitudinal control.',
          'easier maintenance of the engines.',
          'a wing which is less sensitive to flutter.',
          'less influence on lateral/directional stability characteristics such as',
          'dutch roll.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which phenomenon is counteracted with differential aileron deflection?',
        options: [
          'Adverse yaw.',
          'Aileron reversal.',
          'Sensitivity for spiral dive.',
          'Turn co-ordination.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane has a servo tab controlled elevator. What will happen if the elevator jams during flight?',
        options: [
          'Pitch control sense is reversed.',
          'Pitch control is lost.',
          'The servo-tab now works as a negative trim-tab.',
          'The pitch control forces double.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 103',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement regarding the gust load factor on an aeroplane is correct (all other factors of importance being constant) ?',
        options: [
          '1.  Increasing the aspect-ratio of the wing will increase the gust load factor.',
          '2.  Increasing the speed will increase the gust load factor.',
          '1 and 2 are correct.',
          '1 is incorrect and 2 is correct.',
          '1 and 2 are incorrect.',
          '1 is correct and 2 is incorrect.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct?',
        options: [
          'Flap extension causes a reduction in stall speed and the maximum glide',
          'distance.',
          'Flap extension will increase (CL/CD)max thus causing a reduction in the',
          'minimum rate descent.',
          'Flap extension has no influence on the minimum rate of descent, as only',
          'TAS has to be taken into account.',
          'Spoiler extension causes a reduction in stall speed and the minimum',
          'rate of descent, but increases the minimum descent angle.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What will increase the sensitivity to Dutch Roll?',
        options: [
          'An increased static lateral stability.',
          'An increased static directional stability.',
          'A forward movement of the centre of gravity.',
          'An increased anhedral.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 104',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In which phase of the take-off is the aerodynamic effect of ice located on the wing leading edge most critical?',
        options: [
          'The last part of the rotation.',
          'The take-off run.',
          'During climb with all engines operating.',
          'All phases of the take-off are equally critical.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct at the speed for minimum drag (subsonic) ?',
        options: [
          'The gliding angle is minimum (assume zero thrust).',
          'The CL/CD ratio is minimum (assume zero thrust).',
          'Induced drag is greater than the parasite drag.',
          'Propeller aeroplanes fly at that speed at max. endurance.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'From a polar diagram of the entire aeroplane in the clean configuration one can read:',
        options: [
          'the maximum CL/CD ratio and maximum lift coefficient.',
          'the minimum drag and the maximum lift.',
          'the minimum drag coefficient and the maximum lift.',
          'the minimum CL/CD ratio and the minimum drag.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane has a stall speed of 100 kt at a load factor n=1. In a turn with a load factor of n=2, the stall speed is:',
        options: [
          '141 kt',
          '282 kt',
          '70 kt',
          '200 kt',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 105',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The induced angle of attack is the result of:',
        options: [
          'downwash due to tip vortices.',
          'a large local angle of attack in a two dimensional flow.',
          'downwash due to flow separation.',
          'change in direction of flow due to the effective angle of attack.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A horn balance in a control system has the following purpose:',
        options: [
          'to decrease stick forces.',
          'to prevent flutter.',
          'to obtain mass balancing.',
          'to decrease the effective longitudinal dihedral of the aeroplane.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct for a side slip condition at constant speed and side slip angle, where the geometric dihedral of an aeroplane is increased ?',
        options: [
          'the required lateral control force increases.',
          'the required lateral control force decreases.',
          'the required lateral control force does not change.',
          'the stick force per g decreases.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 106',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the position of the elevator in relation to the trimmable horizontal stabiliser of a power assisted aeroplane that is in trim?',
        options: [
          'The position depends on speed, the position of slats and flaps and the',
          'position of the centre of gravity.',
          'The elevator deflection (compared to the stabilizer position) is always',
          'zero.',
          'At a forward CG the elevator is deflected upward and at an aft CG the',
          'elevator is deflected downward.',
          'The elevator is always deflected slightly downwards in order to have',
          'sufficient remaining flare capability.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Mass-balancing of control surfaces is used to:',
        options: [
          'prevent flutter of control surfaces',
          'limit the stick forces.',
          'ensure that the control surfaces are in the mid-position during taxiing .',
          'increase the stick force stability.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane performs a steady horizontal turn with a TAS of 200 kt. The turn radius is 2000 m. The load factor (n) is approximately:',
        options: [
          '1.1.',
          '1.4.',
          '2.0.',
          '1.8.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 107',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Amongst the following factors, which will decrease the ground distance covered during a glide (assume zero power/thrust)?',
        options: [
          'Headwind.',
          'Tailwind.',
          'An increase in aeroplane mass.',
          'A decrease in aeroplane mass.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane exhibits static longitudinal stability, if, when the angle of attack changes:',
        options: [
          'the change in total aeroplane lift acts aft of the centre of gravity.',
          'the change in total aeroplane lift acts through the centre of gravity.',
          'the resulting moment is positive.',
          'the change in wing lift is equal to the change in tail lift.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If an aeroplane exhibits insufficient stick force per g, this problem can be resolved by installing:',
        options: [
          'a bobweight in the control system which pulls the stick forwards.',
          'a spring which pulls the stick backwards.',
          'a spring which pushes the stick forwards.',
          'a bobweight in the control system which pulls the stick backwards.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The flight Mach number is 0.8 and the TAS is 400 kts. The speed of sound is:',
        options: [
          '500 kts',
          '320 kts',
          '480 kts',
          '600 kts',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 108',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The effect of the wing downwash on the static longitudinal stability of an aeroplane is:',
        options: [
          'negative.',
          'negligible.',
          'positive.',
          'smallest at high values of the lift coefficient.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When the air has passed through a normal shock wave the Mach number is less than 1.',
        options: [
          'lower than before but still greater than 1.',
          'equal to 1.',
          'higher than before.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In a slipping turn (nose pointing outwards), compared to a co-ordinated turn, the bank angle (i) and the "ball" or slip indicator (ii) are respectively:',
        options: [
          '(i) too large, (ii) displaced towards the low wing.',
          '(i) too small, (ii) displaced towards the low  wing.',
          '(i) too large, (ii) displaced towards the high wing.',
          '(i) too small, (ii) displaced towards the high wing.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'For a given aeroplane which two main variables determine the value of VMCG?',
        options: [
          'Airport elevation and temperature.',
          'Engine thrust and rudder deflection.',
          'Air density and runway length.',
          'Engine thrust and gear position.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 109',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When the air is passing through a shock wave the static temperature will increase.',
        options: [
          'decrease.',
          'stay constant.',
          'decrease and beyond a certain Mach number start increasing again',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When the air is passing through a shock wave the density will increase.',
        options: [
          'decrease.',
          'stay constant.',
          'decrease and beyond a certain Mach number start increasing again',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A jet transport aeroplane exhibits pitch up when thrust is suddenly increased from an equilibrium condition, because the thrust line is below the:',
        options: [
          'CG.',
          'drag line of action.',
          'neutral point.',
          'centre of pressure.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When air has passed through a shock wave the local speed of sound is increased.',
        options: [
          'not affected',
          'decreased.',
          'decreased and beyond a certain Mach number start increasing again',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 110',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The pitch angle is defined as the angle between the:',
        options: [
          'longitudinal axis and the horizontal plane.',
          'longitudinal axis and the chord line.',
          'chord line and the horizontal plane.',
          'speed vector axis and the longitudinal axis.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If the Mach number of an aeroplane in supersonic flight is increased, the Mach cone angle will:',
        options: [
          'decrease.',
          'increase.',
          'stay constant.',
          'decrease and beyond a certain Mach number start increasing again.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Consider subsonic incompressible airflow through a venturi:',
        options: [
          'I The dynamic pressure in the undisturbed airflow is higher than in the throat.',
          'II The total pressure in the undisturbed airflow is higher than in the throat.',
          'I is incorrect, II is incorrect.',
          'I is incorrect, II is correct.',
          'I is correct, II is correct.',
          'I is correct, II is incorrect.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The loss of total pressure in a shock wave is due to the fact that kinetic energy in the flow is converted into heat energy.',
        options: [
          'the speed reduction is too high.',
          'the static pressure decrease is comparatively high.',
          'the friction in the boundary layer is higher.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 111',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane has the following flap positions: 0°, 15°, 30°, 45°. Slats can also be selected. Generally speaking, which selection provides the highest positive contribution to the CLMAX?',
        options: [
          'The slats from the retracted to the take-off position.',
          'The flaps from 30° to 45°.',
          'The flaps from 0° to 15°.',
          'The flaps from 15° to 30°.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When the air is passing through an expansion wave the local speed of sound will decrease.',
        options: [
          'increase.',
          'stay constant.',
          'decrease and beyond a certain Mach number start increasing again.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane enters a horizontal turn with a load factor n=2 from straight and level flight whilst maintaining constant indicated airspeed. The:',
        options: [
          'lift doubles.',
          'induced drag doubles.',
          'lift becomes four times its original value.',
          'total drag becomes four times its original value.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 112',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct about the gust load on an aeroplane, while all other factors of importance remain constant?',
        options: [
          'I When the mass increases, the gust load increases.',
          'II When the altitude decreases, the gust load increases.',
          'I is incorrect, II is correct.',
          'I is incorrect, II is incorrect.',
          'I is correct, II is correct.',
          'I is correct, II is incorrect.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When the air is passing through an expansion wave the Mach number will increase.',
        options: [
          'decrease.',
          'stay constant.',
          'decrease and beyond a certain Mach number start increasing again.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When the air is passing through an expansion wave the static temperature will decrease.',
        options: [
          'increase.',
          'stay constant.',
          'decrease and beyond a certain Mach number start increasing again.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 113',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane is in a steady horizontal turn at a TAS of 194.4 kt. The turn',
        options: [
          'radius is 1000 m. The bank angle is: (assume g = 10 m/sec2)',
          '45°',
          '30°',
          '50°',
          '60°',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Compared with an oblique shock wave at the same Mach number a normal shock wave has a higher compression.',
        options: [
          'higher expansion.',
          'smaller compression.',
          'smaller expansion.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The manoeuvre stability of a large jet transport aeroplane is 280 N/g. What',
        options: [
          'stick force is required, if the aeroplane is pulled to the limit manoeuvring load',
          'factor from a trimmed horizontal straight and steady flight? (cruise',
          'configuration)',
          '420 N',
          '770 N',
          '630 N',
          '1050 N',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 114',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Upward deflection of a trim tab in the longitudinal control results in:',
        options: [
          'the stick position stability remaining constant.',
          'increasing the stick position stability.',
          'increasing the stick force stability.',
          'the stick force stability remaining constant.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Compared with an oblique shock wave at the same Mach number a normal shock wave has a higher loss in total pressure.',
        options: [
          'higher total pressure.',
          'higher total temperature.',
          'lower static temperature.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct?',
        options: [
          'I Stick force per g is independent of altitude.',
          'II Stick force per g increases when the centre of gravity moves forward.',
          'I is incorrect, II is correct.',
          'I is correct, II is correct.',
          'I is correct, II is incorrect.',
          'I is incorrect, II is incorrect',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 115',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In case of supersonic flow retarded by a normal shock wave a high efficiency (low loss in total pressure) can be obtained if the Mach number in front of the shock is small but still supersonic.',
        options: [
          'high (supersonic).',
          'lower than 1.',
          'exactly 1.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The regime of flight from the critical Mach number up to approximately M = 1.3 is called the:',
        options: [
          'transonic range.',
          'supersonic range.',
          'hypersonic range.',
          'subsonic range.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The following factors increase stall speed:',
        options: [
          'an increase in load factor, a forward c.g. shift, decrease in thrust.',
          'a higher weight, selecting a higher flap setting, a forward c .g. shift.',
          'increasing bank angle, increasing thrust, slat extension.',
          'a lower weight, decreasing bank angle, a smaller flapsetting.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Just above the critical Mach number the first evidence of a shock wave will appear at the upper side of the wing.',
        options: [
          'lower side of the wing.',
          'leading edge of the wing.',
          'trailing edge of the wing.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 116',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct?',
        options: [
          'As the angle of attack increases, the stagnation point on the wing\'s',
          'profile moves downwards.',
          'The centre of pressure is the point on the wing\'s leading edge where the',
          'airflow splits up.',
          'The stagnation point is another name for centre of pressure.',
          'The stagnation point is always situated on the chordline, the centre of',
          'pressure is not.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement about a jet transport aeroplane is correct during take-off with the cg at the forward limit and the trimmable horizontal stabiliser (THS) positioned at the maximum allowable aeroplane nose down position?',
        options: [
          'The rotation will require extra stick force.',
          'If the THS position is just within the limits of the green band, the take off',
          'warning system will be activated.',
          'Early nose wheel raising will take place.',
          'Rotation will be normal.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'As the Mach number increases in straight and level flight, a shock wave on the upper surface of the wing will:',
        options: [
          'move towards the trailing edge.',
          'move towards the leading edge.',
          'not move.',
          'disappear.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 117',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Shock induced separation results in decreasing lift.',
        options: [
          'constant lift.',
          'increasing lift.',
          'decreasing drag.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When flutter damping of control surfaces is obtained by mass balancing, these weights will be located with respect to the hinge of the control surface:',
        options: [
          'in front of the hinge.',
          'below the hinge.',
          'above the hinge.',
          'behind the hinge.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In the transonic range lift will decrease at the shock stall due to the separation of the boundary layer at the shock waves.',
        options: [
          'attachment of the shock wave on the trailing edge of the wing.',
          'first appearance of a shock wave at the upper side of the wing.',
          'appearance of the bow wave.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'How can the designer of an aeroplane with straight wings increase the static lateral stability?',
        options: [
          'By increasing the aspect ratio of the vertical stabiliser, whilst',
          'maintaining a constant area.',
          'By fitting a ventral fin (a fin at the under side of the aeroplane).',
          'By applying wing twist.',
          'By increasing anhedral.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 118',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Is a transport aeroplane allowed to fly at a higher Mach number than the \'buffet-onset\' Mach number in 1g flight?',
        options: [
          'No, this is not acceptable',
          'Yes, this causes no problems.',
          'Yes, but only during approach.',
          'Yes, if you want to fly fast at very high altitudes.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Positive static longitudinal stability means that a:',
        options: [
          'nose-down moment occurs after encountering an up-gust.',
          'nose-up moment occurs with a speed change at constant angle of',
          'attack.',
          'nose-down moment occurs with a speed change at constant angle of',
          'attack.',
          'nose-up moment occurs after encountering an up-gust.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'To increase the critical Mach number a conventional aerofoil should have a low thickness to chord ratio.',
        options: [
          'have a large camber.',
          'be used with a high angle of attack.',
          'have a large leading edge radius.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 119',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement in respect of a trimmable horizontal stabiliser is correct?',
        options: [
          'An aeroplane with a forward centre of gravity requires the stabiliser',
          'leading edge to be lower than for one with an aft centre of gravity in the',
          'same trimmed condition.',
          'Because take-off speeds do not vary with centre of gravity location, the',
          'need for stabiliser adjustment is dependant on flap position only.',
          'An aeroplane with a forward centre of gravity requires the stabiliser',
          'leading edge to be higher than for one  with an aft centre of gravity in the',
          'same trimmed condition.',
          'At the forward centre of gravity limit , stabiliser trim is adjusted fully',
          'nose down to obtain maximum elevator authority at rotation during',
          'take-off.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The critical Mach number can be increased by sweepback of the wings.',
        options: [
          'positive dihedral of the wings.',
          'a T-tail.',
          'an increase in wing aspect ratio.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which definition of propeller parameters is correct?',
        options: [
          'Geometric pitch is the theoretical distance a propeller blade element',
          'would travel in a forward direction during one revolution.',
          'Blade angle is the angle between the blade chord line and the propeller',
          'axis.',
          'Angle of attack is the angle between the blade chord line and the',
          'propeller vertical plane.',
          'Critical tip velocity is the propeller speed at which flow separation first',
          'occurs at some part of the blade.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 120',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Some aeroplanes have a \'waist\' or \'coke bottle\' contoured fuselage. This is done to apply area rule.',
        options: [
          'increase the strength of the wing root junction.',
          'fit the engine intakes better to the fuselage.',
          'improve the low speed characteristics.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The angle of attack for a propeller blade is the angle between the blade chord line and the:',
        options: [
          'local air speed vector.',
          'direction of propeller axis.',
          'aeroplane heading.',
          'principal direction of the propeller blade.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Given two identical aeroplanes with wing mounted engines, one fitted with jet engines and the other with counter rotating propellers, what happens following an engine failure?',
        options: [
          'More roll tendency for the propeller aeroplane.',
          'The same yaw tendency for both aeroplanes regardless of left or right',
          'engine failure.',
          'The same roll tendency for both aeroplanes.',
          'Less roll tendency for the propeller aeroplane.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 121',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Vortex generators mounted on the upper wing surface will decrease the shock wave induced separation.',
        options: [
          'decrease the interference drag of the trailing edge flaps.',
          'decrease the stalling speed by increasing spanwise flow on the wing.',
          'increase the effectiveness of the spoiler due to increase in parasite drag.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The application of the area rule on aeroplane design will decrease the wave drag.',
        options: [
          'skin friction drag.',
          'induced drag.',
          'form drag.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The stall speed (IAS) will change according to the following factors:',
        options: [
          'May increase with altitude, especially high altitude, will increase during',
          'icing conditions and will increase when the cg moves forward',
          'May increase when the cg moves forward, with higher altitude and due',
          'to the slipstream from a propeller on an engine located forward of the',
          'wing',
          'Will increase in a turn, higher temperature and will increase when the cg',
          'moves aft',
          'Will increase with increased load factor, more flaps and increased bank',
          'angle in a turn',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 122',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The stalling speed in IAS will change according to the following factors:',
        options: [
          'Increase during turn, increased mass and forward c.g. location',
          'Decrease in a forward c.g. location, higher altitude and due to the slip',
          'stream from a propeller on an engine located forward of the wing',
          'Increase with increased load factor, icing conditions and an aft c.g.',
          'location',
          'Increase with increased load factor, more flaps but will not increase due',
          'to the bank angle in a turn',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Tuck under will happen only above the critical Mach number.',
        options: [
          'only at the critical Mach number.',
          'only below the critical Mach number.',
          'above or below the critical Mach number depending on the angle of',
          'attack.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The stalling speed in IAS will change according to the following factors:',
        options: [
          'May increase during turbulence and will always increase when banking',
          'in a turn',
          'Will increase during turn, increased mass and an aft c.g. location',
          'Will decrease with a forward c.g. location, lower altitude and due to the',
          'slip stream from a propeller on an engine located forward of the wing',
          'Will increase with increased load factor, icing conditions and more flaps',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 123',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'High speed buffet is induced by boundary layer separation due to shock wave formation.',
        options: [
          'boundary layer control.',
          'expansion waves on the wing upper surface.',
          'a shift of the centre of gravity.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements is true?',
        options: [
          'Flight in severe turbulence may lead to a stall and/or structural',
          'limitations being exceeded',
          'Flap extension in severe turbulence at constant speed increases both',
          'the stall speed and the structural limitation margins',
          'By increasing the flap setting in severe turbulence at constant speed the',
          'stall speed will be reduced and the risk for exceeding the structural',
          'limits will be decreased',
          'Flap extension in severe turbulence at constant speed moves the centre',
          'of pressure aft, which increases the structural limitation margins',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A conventional stabiliser on a stable aeroplane in a normal cruise condition:',
        options: [
          '1 - always provides negative lift.',
          '2 - contributes to the total lift of the aeroplane.',
          '3 - may stall before the wing, in icing conditions, with large flap settings,',
          'unless adequate design and/or operational precautions are taken.',
          '4 - is necessary to balance the total pitch moment of the aeroplane.',
          'Which of the following lists all the correct statements ?',
          '2, 3 and 4',
          '1, 3 and 4',
          '1 and 3',
          '1 and 4',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 124',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statements are correct for a straight steady climb?',
        options: [
          '1 - lift is less than weight.',
          '2 - lift is greater than weight.',
          '3 - load factor is less than 1.',
          '4 - load factor is greater than 1.',
          '5 - lift is equal to weight.',
          '6 - load factor is equal to 1.',
          'Which of the following lists all the correct statements ?',
          '1 and 3',
          '2 and 4',
          '5 and 6',
          '1 and 6',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'If a symmetrical aerofoil is accelerated from subsonic to supersonic speed the aerodynamic centre will move aft to the mid of the chord.',
        options: [
          'aft to the trailing edge.',
          'forward to the leading edge.',
          'forward to the mid of the chord.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Given an initial condition in straight and level flight with a speed of 1.4 VS.',
        options: [
          'The maximum bank angle attainable without stalling in a steady co-ordinated',
          'turn, whilst maintaining speed and altitude, is approximately:',
          '60°.',
          '44°.',
          '32°.',
          '30°.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 125',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A 50 ton twin engine aeroplane performs a straight, steady, wings level climb.',
        options: [
          'If the lift/drag ratio is 12 and the thrust is 60 000N per engine, the climb',
          'gradient is: (assume g = 10m/s²)',
          '15.7%.',
          '3.7%.',
          '24%.',
          '12%.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane in straight and level flight is subjected to a strong vertical gust.',
        options: [
          'The point on the wing, where the instantaneous variation in wing lift',
          'effectively acts is known as the:',
          'aerodynamic centre of the wing.',
          'centre of thrust.',
          'centre of gravity.',
          'neutral point.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In a straight steady descent, which of the following statements is correct ?',
        options: [
          'Lift is less than weight, load factor is less than 1.',
          'Lift is less than weight, load factor is equal to 1.',
          'Lift is equal to weight, load factor is equal to 1.',
          'Lift is equal to weight, load factor is less than 1.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'VLE is defined as the:',
        options: [
          'maximum landing gear extended speed.',
          'maximum speed at which the landing gear may be extended or retracted.',
          'maximum flap extended speed.',
          'maximum authorised speed.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 126',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When Fowler type trailing edge flaps are extended at a constant angle of attack, the following changes will occur:',
        options: [
          'CL and CD increase.',
          'CL increases and the centre of pressure moves forward.',
          'CL increases and CD remains constant.',
          'CD decreases and the centre of pressure moves aft.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane has a stall speed of 100 kt at a mass of 1000 kg. If the mass is increased to 2000 kg, the new value of the stall speed will be:',
        options: [
          '141 kt.',
          '200 kt.',
          '150 kt.',
          '123 kt.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'At what speed does the front of a shock wave move across the earth\'s surface?',
        options: [
          'The ground speed of the aeroplane.',
          'The speed of sound at ground level.',
          'The speed of sound at flight level.',
          'The true air speed of the aeroplane.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'When an aeroplane performs a straight steady climb with a 20% climb gradient, the load factor is equal to:',
        options: [
          '0.98.',
          '1.02.',
          '1.',
          '0.83.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 127',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In a convergent tube with an incompressible sub-sonic airflow, the following pressure changes will occur:',
        options: [
          'Ps = static pressure.',
          'Pdyn = dynamic pressure.',
          'Ptot = total pressure.',
          'Ps decreases, Pdyn increases, Ptot remains constant.',
          'Ps increases, Pdyn decreases, Ptot remains constant.',
          'Ps decreases, Pdyn increases, static temperature increases.',
          'Ps decreases, Ptot increases, static temperature decreases.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane climbs to cruising level with a constant pitch attitude and maximum climb thrust, (assume no supercharger).',
        options: [
          'How do the following variables change during the climb? (gamma = flight path',
          'angle)',
          'gamma decreases, angle of attack increases, IAS decreases.',
          'gamma decreases, angle of attack increases, IAS remains constant.',
          'gamma decreases, angle of attack remains constant, IAS decreases.',
          'gamma remains constant, angle of attack remains constant, IAS',
          'decreases.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The point, where the single resultant aerodynamic force acts on an aerofoil, is called:',
        options: [
          'centre of pressure.',
          'centre of gravity.',
          'neutral point.',
          'aerodynamic centre.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 128',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Mach buffet occurs:',
        options: [
          'at the Mach Number at which shockwave induced boundary layer',
          'separation occurs.',
          'when the Mach Number has increased to Mcrit.',
          'directly after exceeding Mcrit.',
          'when the stalling angle of attack is exceeded.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement about minimum control speed is correct?',
        options: [
          'VMCA depends on the airport density altitude, and the location of the',
          'engine on the aeroplane (aft fuselage or wing).',
          'The nose wheel steering control may used to determine VMCG.',
          'Crosswind is taken into account to determine VMCG.',
          'VMCA may not be lower than VMCL',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The manoeuvring speed VA expressed in indicated airspeed, of a transport aeroplane:',
        options: [
          'depends on aeroplane mass and pressure altitude.',
          'is a constant value.',
          'is independent of aeroplane mass, but dependent on pressure altitude.',
          'depends on aeroplane mass and is independent of pressure altitude.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'How can wing flutter be prevented?',
        options: [
          'By locating mass in front of the torsion axis of the wing.',
          'By increasing the aspect ratio of the wing.',
          'By mounting the engines on the fuselage.',
          'By installing the fuel tanks in the fuselage.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 129',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements is correct ?',
        options: [
          'I When the critical engine fails during take-off the speed VMCL can be limiting.',
          'II The speed VMCL is always limited by maximum rudder deflection.',
          'I is incorrect, II is incorrect',
          'I is correct, II is correct',
          'I is correct, II is incorrect',
          'I is incorrect, II is correct',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements is correct ?',
        options: [
          'I When the critical engine fails during take-off the speed VMCL can be limiting.',
          'II The speed VMCL can be limited by the available maximum roll rate',
          'I is incorrect, II is correct',
          'I is correct, II is correct',
          'I is incorrect, II is incorrect',
          'I is correct, II is incorrect',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which of the following statements is correct ?',
        options: [
          'I VMCL is the minimum control speed in the landing configuration.',
          'II The speed VMCL can be limited by the available maximum roll rate',
          'I is correct, II is correct',
          'I is incorrect, II is incorrect',
          'I is correct, II is incorrect',
          'I is incorrect, II is correct',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 130',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The speed for minimum glide angle occurs at an angle of attack that corresponds to:',
        options: [
          '(assume zero thrust;  ^ … denotes power of …)',
          '(CL/CD)max',
          'CLmax',
          '(CL/CD^2)max',
          '(CL^3/CD^2)max',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'How does VMCG change with increasing field elevation and temperature ?',
        options: [
          'decreases, because the engine thrust decreases.',
          'decreases, because VMCG is expressed in IAS which decreases with',
          'constant TAS  and decreasing density',
          'increases, because at a lower density a larger IAS is necessary to',
          'generate the required rudder force',
          'increases, because VMCG is related to V1 and VR and those speeds',
          'increase if the density decreases',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'What is the influence of decreasing aeroplane weight on Mcrit at constant IAS ?',
        options: [
          'Mcrit increases as a result of flying at a smaller angle of attack.',
          'Mcrit increases as a result of compressibility effects.',
          'Mcrit decreases.',
          'Mcrit decreases as a result of flying at a greater angle of attack.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 131',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Assuming ISA conditions, which statement with respect to the climb is correct ?',
        options: [
          'At constant IAS the Mach number increases',
          'At constant IAS the TAS decreases',
          'At constant Mach number the IAS increases',
          'At constant TAS the Mach number decreases',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'In case the Mach trimmer fails:',
        options: [
          'the Mach number must be limited.',
          'try to relocate the centre-of-gravity aft.',
          'the speed must be kept constant.',
          'the aeroplane weight must be limited.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'During a climb at a constant IAS, the Mach Number:',
        options: [
          'increase.',
          'decrease initially and increase subsequently.',
          'remain constant.',
          'increase initially and remain constant subsequently.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'A Machtrimmer:',
        options: [
          'corrects insufficient stick force stability at high Mach Numbers.',
          'increases the stick force per g at high Mach Numbers.',
          'is necessary for compensation of the autopilot at high Mach Numbers.',
          'has no effect on the shape of the elevator position versus speed (IAS)',
          'curve for a fully hydraulic controlled aeroplane.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 132',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The speed range between high- and low speed buffet:',
        options: [
          'increases during a descent at a constant IAS.',
          'increases during climb.',
          'decreases during a descent at a constant Mach number.',
          'is always positive at Mach numbers below MMO.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Whilst flying at a constant IAS and at n = 1, as the aeroplane mass decreases the value of Mcrit:',
        options: [
          'increases.',
          'remains constant.',
          'is independent of the angle of attack.',
          'decreases.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The positive manoeuvring limit load factor for a light aeroplane in the utility category in the clean configuration is:',
        options: [
          '4.4',
          '2.5',
          '3.8',
          '6.0',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 133',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct about an expansion wave in supersonic flow ?',
        options: [
          '1. The static temperature in front of an expansion wave is higher than the',
          'static temperature behind it.',
          '2. The speed in front of an expansion wave is higher than the speed behind it.',
          '1 is correct and 2 is incorrect.',
          '1 and 2 are correct.',
          '1 is incorrect and 2 is correct.',
          '1 and 2 are incorrect.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement with respect to the speed of sound is correct ?',
        options: [
          'Varies with the square root of the absolute temperature.',
          'Increases always if the density of the air decreases.',
          'Is independent of altitude.',
          'Doubles if the temperature increases from 9° to 36° Centigrade.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'The positive manoeuvring limit load factor for a large jet transport aeroplane with flaps extended is:',
        options: [
          '2.0',
          '1.5',
          '2.5',
          '3.75',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 134',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'During the take-off roll with a strong crosswind from the left, a four engine jet aeroplane with wing mounted engines experiences an engine failure above V1. Which engine causes the greatest control problem?',
        options: [
          'The left outboard engine.',
          'The right outboard engine.',
          'The right inboard engine.',
          'The left inboard engine.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'Which statement is correct regarding a shockwave on a lift generating wing?',
        options: [
          'It moves slightly aft when an aileron is deflected downward.',
          'It reaches its highest strength when flying at the critical Mach number.',
          'It moves forward when the Mach number is increased.',
          'It is located at the greatest wing thickness when the aeroplane reaches',
          'the speed of sound.',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'An aeroplane should be equipped with a Mach trimmer, if:',
        options: [
          'at transonic Mach numbers the aeroplane demonstrates unconventional',
          'elevator stick force characteristics.',
          'stick force stability is independent of the airspeed and -altitude.',
          'stick force per g strongly decreases at low Mach numbers.',
          'at high airspeed and low altitude the aeroplane demonstrates',
          'unconventional elevator stick force characteristics.',
          'TM-Editor',
          '081-Principles of Flight-A.LTM',
          '25.04.2016',
          'Seite 135',
        ],
        correctAnswer: 0,
      ),
      Question(
        question: 'How will the density and static temperature change in a supersonic flow from a position in front of a shock wave to behind it ?',
        options: [
          'Density will increase, static temperature will increase.',
          'Density will increase, static temperature will decrease.',
          'Density will decrease, static temperature will increase.',
          'Density will decrease, static temperature will decrease.',
          '© LPLUS GmbH 2007',
        ],
        correctAnswer: 0,
      ),
    ];
  }
}